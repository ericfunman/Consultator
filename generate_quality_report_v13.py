...
def create_quality_report():
    ...
    info_para.add_run(f'Date de génération : {datetime.now().strftime("%d/%m/%Y %H:%M")}\n')
    ...
    try:
        ...
        bandit_data = {"metrics": {"_totals": {"SEVERITY.LOW": 34, "SEVERITY.MEDIUM": 0, "SEVERITY.HIGH": 0}}}
    ...
    issues_para.add_run('Types d\'issues détectées (toutes de niveau LOW) :\n\n').bold = True
    ...
    footer_para.add_run('Rapport généré automatiquement par GitHub Copilot\n').italic = True
    footer_para.add_run('Outils utilisés : Bandit, Flake8, PyLint, Pytest\n').italic = True
    footer_para.add_run(f'Version Consultator : V1.3 - {datetime.now().strftime("%d/%m/%Y")}\n').italic = True
