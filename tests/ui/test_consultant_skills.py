...
from app.pages_modules.consultant_skills import (
    show_consultant_skills, get_niveau_label, show_skills_statistics,
    show_add_skill_form, add_skill_to_consultant, show_edit_skill_form,
    update_consultant_skill, delete_skill, show_skills_analysis, show_skills_evolution
)
...
class TestConsultantSkills(BaseUITest):
    ...
    def test_show_add_skill_form_success(self, mock_session, mock_submit, mock_cols, mock_num_input,
                                       mock_checkbox, mock_slider, mock_selectbox, mock_form, mock_md):
        ...
        mock_existing_cc = []
        mock_available_comp = [MagicMock(id=1, nom="Python", categorie="Technique"),
        ...
        with patch('app.pages_modules.consultant_skills.add_skill_to_consultant', return_value=True) as mock_add_skill:
    ...
    @patch('app.pages_modules.consultant_skills.st.form_submit_button')
    @patch('app.pages_modules.consultant_skills.get_database_session')
    def test_show_edit_skill_form_success(self, mock_session, mock_submit, mock_cols, mock_num_input,
                                        mock_checkbox, mock_slider, mock_form, mock_md):
        ...
        mock_cc = MagicMock(id=1, competence=mock_competence, niveau=3,
                           certification=True, annees_experience=4)
        ...
        with patch('app.pages_modules.consultant_skills.update_consultant_skill', return_value=True) as mock_update_skill:
    ...
    @patch('app.pages_modules.consultant_skills.st.warning')
    @patch('app.pages_modules.consultant_skills.st.info')
    def test_show_skills_analysis_with_data(self, mock_info, mock_warning, mock_success,
                                          mock_write, mock_cols, mock_md):
    ...
    def test_show_skills_evolution_with_data(self, mock_session, mock_write, mock_cols, mock_md):
        ...
        mock_session_instance.query.return_value.join.return_value.filter.return_value.order_by.return_value.all.return_value = [mock_cc1, mock_cc2]
    ...
    @patch('app.pages_modules.consultant_skills.st.markdown')
    @patch('app.pages_modules.consultant_skills.imports_ok', True)
    def test_show_consultant_skills_with_full_data(self, mock_md, mock_columns, mock_button, mock_metric, mock_write, mock_expander, mock_session):
        ...
        mock_cc1 = MagicMock(id=1, competence=mock_competence1, niveau=4,
                            annees_experience=5, certification=True,
                            date_acquisition=None)
        mock_cc2 = MagicMock(id=2, competence=mock_competence2, niveau=3,
                            annees_experience=3, certification=False,
                            date_acquisition=None)
        ...
        # Mock UI components
        mock_col1, mock_col2, mock_col3, mock_col4 = MagicMock(), MagicMock(), MagicMock(), MagicMock()
        mock_columns.side_effect = lambda n: (mock_col1, mock_col2, mock_col3, mock_col4)[:n]  # Return n columns

        mock_button.return_value = False  # No button clicks

        with patch('app.pages_modules.consultant_skills.st.session_state') as mock_state, \
    ...
    def test_show_consultant_skills_with_different_categories(self):
        ...
        with patch('app.pages_modules.consultant_skills.imports_ok', True):
            with patch('app.pages_modules.consultant_skills.get_database_session') as mock_session:
                ...
                mock_cc1 = MagicMock(id=1, competence=mock_comp1, niveau=4, annees_experience=5, certification=True)
                mock_cc2 = MagicMock(id=2, competence=mock_comp2, niveau=3, annees_experience=3, certification=False)
                mock_cc3 = MagicMock(id=3, competence=mock_comp3, niveau=4, annees_experience=2, certification=True)

                mock_session_instance.query.return_value.join.return_value.filter.return_value.all.return_value = [
                    mock_cc1, mock_cc2, mock_cc3
                ]
                ...
                with patch('app.pages_modules.consultant_skills.st.markdown'):
                    with patch('app.pages_modules.consultant_skills.st.columns'):
                        with patch('app.pages_modules.consultant_skills.st.button', return_value=False):
    ...
    def test_integration_with_pandas_dataframe(self):
        ...
        with patch('app.pages_modules.consultant_skills.imports_ok', True):
            with patch('app.pages_modules.consultant_skills.get_database_session') as mock_session:
                ...
                mock_cc = MagicMock(id=1, competence=mock_competence, niveau=4,
                                   annees_experience=5, certification=True)

                mock_session_instance.query.return_value.join.return_value.filter.return_value.all.return_value = [mock_cc]
                            ...
                            with patch('app.pages_modules.consultant_skills.st.metric'):
                                with patch('app.pages_modules.consultant_skills.st.write'):
                                    ...
                                    with patch('pandas.DataFrame') as mock_df:
                                        ...
                                        expected_columns = ["Compétence", "Niveau", "Expérience", "Certification", "Actions"]
