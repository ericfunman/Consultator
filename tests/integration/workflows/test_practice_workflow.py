...
def sample_consultants_for_practice():
    ...
    consultants_data = [
        {
            "prenom": "Alice", "nom": "Data", "email": "alice.data@test.com",
            "salaire_actuel": 65000, "practice_id": 1, "disponibilite": True,
            "grade": "Expert", "societe": "DataCorp"
        },
        {
            "prenom": "Bob", "nom": "AI", "email": "bob.ai@test.com",
            "salaire_actuel": 60000, "practice_id": 1, "disponibilite": True,
            "grade": "Senior", "societe": "DataCorp"
        },
        {
            "prenom": "Claire", "nom": "ML", "email": "claire.ml@test.com",
            "salaire_actuel": 55000, "practice_id": 1, "disponibilite": False,
            "grade": "Senior", "societe": "DataCorp"
        }
    ]
...
class TestPracticeWorkflowIntegration:
    ...
    def test_complete_practice_workflow(self, sample_practice_data, sample_consultants_for_practice):
        ...
        try:
            ...
            with get_database_session() as session:
                created_practice = session.query(Practice).filter(Practice.id == practice_id).first()
                assert created_practice is not None
                assert created_practice.nom == sample_practice_data["nom"]
                assert created_practice.description == sample_practice_data["description"]
            ...
            for consultant_id in consultant_ids:
                result = ConsultantService.update_consultant(consultant_id, {"practice_id": practice_id})
                assert result is True

            # Vérifier les assignations
            with get_database_session() as session:
                consultants_in_practice = session.query(Consultant).filter(
            ...
            practice_consultants = [c for c in all_consultants if c.get("practice_name") == sample_practice_data["nom"]]
            ...
            print(f"📊 Statistiques practice: {len(practice_consultants)} consultants, "
            ...
            search_results = ConsultantService.search_consultants_optimized("", page=1, per_page=50)
            practice_results = [c for c in search_results if c.get("practice_name") == sample_practice_data["nom"]]
            ...
            with get_database_session() as session:
                ...
                practice.description = "Description mise à jour pour les tests d'intégration"
                practice.responsable = "Dr. Jean Martin"
                session.commit()

            # Vérifier les modifications
            with get_database_session() as session:
                updated_practice = session.query(Practice).filter(Practice.id == practice_id).first()
                assert updated_practice.description == "Description mise à jour pour les tests d'intégration"
            ...
            for consultant_id in consultant_ids:
                try:
                    ConsultantService.update_consultant(consultant_id, {"practice_id": None})
                except:
                    pass
    ...
    def test_practice_statistics_workflow(self):
        ...
        try:
            ...
            consultants_data = [
                # Frontend
                {"prenom": "Anna", "nom": "React", "email": "anna.react@test.com", "salaire_actuel": 55000, "practice_id": practice_ids[0], "disponibilite": True},
                {"prenom": "Ben", "nom": "Vue", "email": "ben.vue@test.com", "salaire_actuel": 52000, "practice_id": practice_ids[0], "disponibilite": False},
                # Backend
                {"prenom": "Charlie", "nom": "Python", "email": "charlie.python@test.com", "salaire_actuel": 60000, "practice_id": practice_ids[1], "disponibilite": True},
                {"prenom": "Diana", "nom": "Java", "email": "diana.java@test.com", "salaire_actuel": 58000, "practice_id": practice_ids[1], "disponibilite": True},
                {"prenom": "Eve", "nom": "Node", "email": "eve.node@test.com", "salaire_actuel": 54000, "practice_id": practice_ids[1], "disponibilite": False},
            ]
            ...
            frontend_consultants = [c for c in all_consultants if c.get("practice_name") == "Frontend Development"]
            backend_consultants = [c for c in all_consultants if c.get("practice_name") == "Backend Development"]

            # Vérifications Frontend
            assert len(frontend_consultants) == 2
            frontend_available = sum(1 for c in frontend_consultants if c["disponibilite"])
            assert frontend_available == 1  # Anna disponible, Ben non disponible

            # Vérifications Backend
            assert len(backend_consultants) == 3
            backend_available = sum(1 for c in backend_consultants if c["disponibilite"])
            assert backend_available == 2  # Charlie et Diana disponibles, Eve non

            # Calcul des salaires moyens
            frontend_avg_salary = sum(c["salaire_actuel"] for c in frontend_consultants) / len(frontend_consultants)
    ...
    def test_practice_consultant_reassignment(self):
        ...
        try:
            ...
            consultant_data = {
                "prenom": "Test", "nom": "Reassignment", "email": "test.reassignment@test.com",
                "salaire_actuel": 50000, "practice_id": practice_ids[0], "disponibilite": True,
                "grade": "Senior", "societe": "TestCorp"
            }

            result = ConsultantService.create_consultant(consultant_data)
            assert result is True
            consultant = ConsultantService.get_consultant_by_email(consultant_data["email"])
            consultant_id = consultant.id

            # Vérifier l'assignation initiale
            consultant_with_stats = ConsultantService.get_consultant_with_stats(consultant_id)
            assert consultant_with_stats["id"] is not None  # Vérifier que le consultant existe

            # Réassigner à la nouvelle practice
            result = ConsultantService.update_consultant(consultant_id, {"practice_id": practice_ids[1]})
            assert result is True
            ...
            old_practice_consultants = [c for c in all_consultants if c.get("practice_name") == "Old Practice"]
            new_practice_consultants = [c for c in all_consultants if c.get("practice_name") == "New Practice"]

            assert len(old_practice_consultants) == 0  # Plus de consultants dans l'ancienne practice
            assert len(new_practice_consultants) == 1  # Un consultant dans la nouvelle practice
