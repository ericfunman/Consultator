...
class TestConsultantsUltraCoverage:
    ...
    def test_show_cv_analysis_fullwidth_with_analysis(self, mock_st):
        ...
        with patch("app.pages_modules.consultants.show_cv_missions") as mock_show_missions, patch(
            "app.pages_modules.consultants.show_cv_skills"
        ) as mock_show_skills, patch("app.pages_modules.consultants.show_cv_summary") as mock_show_summary, patch(
            "app.pages_modules.consultants.show_cv_actions"
        ) as mock_show_actions:
            ...
            mock_st.tabs.assert_called_once_with(["📋 Missions", "🛠️ Compétences", "📊 Résumé", "💾 Actions"])
    ...
    def test_show_consultant_profile_success(self, mock_display_metrics, mock_display_header, mock_load_data, mock_st):
    ...
    def test_load_consultant_for_edit(
        self, mock_get_practice_id, mock_extract_bm, mock_load_relations, mock_get_session
    ):
    ...
    def test_render_basic_consultant_fields(
        self, mock_display_status, mock_render_professional, mock_render_company, mock_render_basic, mock_st
    ):
    ...
    def test_render_company_history_fields(
        self, mock_render_date_mission, mock_render_date_sortie, mock_render_date_entree, mock_render_societe, mock_st
    ):
        ...
        assert result == (
            "Quanteam",
            mock_render_date_entree.return_value,
            mock_render_date_sortie.return_value,
            mock_render_date_mission.return_value,
        )
    ...
    def test_render_societe_field(self, mock_st):
        ...
        mock_st.selectbox.assert_called_once_with(
            "🏢 Société", options=["Quanteam", "Asigma"], index=0  # Quanteam est à l'index 0
        )
    ...
    def test_render_date_entree_field(self, mock_st):
        ...
        mock_st.date_input.assert_called_once_with(
            "📅 Date d'entrée société", value=mock_date, help="Date d'entrée dans la société"
        )
    ...
    def test_render_date_sortie_field(self, mock_st):
        ...
        mock_st.date_input.assert_called_once_with(
            "📅 Date de sortie société (optionnel)", value=mock_date, help="Laissez vide si encore en poste"
        )
        assert result == mock_st.date_input.return_value
    ...
    def test_render_date_premiere_mission_field(self, mock_st):
        ...
        mock_st.date_input.assert_called_once_with(
            "🚀 Date première mission (optionnel)", value=mock_date, help="Date de début de la première mission"
        )
        assert result == mock_st.date_input.return_value
    ...
    def test_load_and_ensure_salary_history(self, mock_datetime, mock_get_session):
        ...
        with patch("app.pages_modules.consultants._should_add_initial_salary_entry") as mock_should_add, patch(
            "app.pages_modules.consultants._add_initial_salary_entry"
        ) as mock_add:
    ...
    def test_display_salary_history_content(self, mock_st):
        ...
        with patch("app.pages_modules.consultants._display_salary_list") as mock_display_list, patch(
            "app.pages_modules.consultants._update_current_salary_if_needed"
        ) as mock_update, patch("app.pages_modules.consultants._display_salary_evolution_chart") as mock_chart:
