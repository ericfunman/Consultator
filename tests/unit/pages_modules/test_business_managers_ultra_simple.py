...
class TestBusinessManagersUltraSimple(unittest.TestCase):
    ...
    def test_display_bm_metrics_ultra_simple(self, mock_markdown, mock_write, mock_columns):
        ...
        bms_data_from_service = [{"actif": True, "nom": "Martin"}, {"actif": False, "nom": "Dupont"}]
        bms_data = [{"Consultants actuels": 5}, {"Consultants actuels": 3}]
    ...
    def test_display_business_managers_list_headers_ultra_simple(
        self, mock_button, mock_write, mock_header, mock_columns
    ):
        ...
        with patch("app.pages_modules.business_managers.BusinessManagerService") as mock_service:
            mock_service.get_all_business_managers.return_value = [
                type(
                    "MockBM",
                    (),
                    {"id": 1, "prenom": "Marie", "nom": "Martin", "email": "marie@test.com", "telephone": "0123456789"},
                )()
            ]
    ...
    @patch("streamlit.columns")
    def test_form_submission_patterns_ultra_simple(
        self, mock_columns, mock_success, mock_submit, mock_text_input, mock_form
    ):
