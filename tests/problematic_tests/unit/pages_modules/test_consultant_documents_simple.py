...
class TestBasicConsultantDocuments:
    ...
    @patch("app.pages_modules.consultant_documents.st")
    def test_show_consultant_documents_with_consultant_no_documents(
        self, mock_st, mock_session, mock_show_stats
    ):
        """Test avec consultant valide mais sans documents"""
        # Mock imports_ok to True and Document class
        with patch("app.pages_modules.consultant_documents.imports_ok", True), patch(
            "app.pages_modules.consultant_documents.Document", create=True
        ):
            ...
            mock_st.info.assert_called_with(
                "ℹ️ Aucun document trouvé pour ce consultant"
            )

    @patch("app.pages_modules.consultant_documents.show_documents_statistics")
    @patch("app.pages_modules.consultant_documents.get_database_session")
    @patch("app.pages_modules.consultant_documents.st")
    def test_show_consultant_documents_with_documents(
        self, mock_st, mock_session, mock_show_stats
    ):
        """Test avec consultant valide et documents"""
        # Mock imports_ok to True
        with patch("app.pages_modules.consultant_documents.imports_ok", True), patch(
            "app.pages_modules.consultant_documents.show_document_details"
        ) as mock_show_details:
