...
def analyze_consultant_service_performance():
    ...
    try:
        ...
        for i in range(num_tests):
            result, exec_time = measure_time(ConsultantService.get_all_consultants, 1, 50)
        ...
        for i in range(num_tests):
            result, exec_time = measure_time(ConsultantService.search_consultants_optimized, 'Jean')
        ...
        for i in range(num_tests):
            result, exec_time = measure_time(ConsultantService.get_all_consultants_objects, 1, 50)
        ...
        for i in range(num_tests):
            result, exec_time = measure_time(ConsultantService.search_consultants_optimized, 'Abdallah')
        ...
        for i in range(3):  # Moins de tests car plus lourd
            result, exec_time = measure_time(ConsultantService.get_all_consultants, 1, 200)
        ...
        for i, (test_name, avg_time, description) in enumerate(performance_data, 1):
            status = "🚀" if avg_time < 0.1 else "⚡" if avg_time < 0.5 else "🐌" if avg_time > 1.0 else "✅"
...
def analyze_database_performance():
    ...
    try:
        ...
        from database.models import Consultant, Mission, Practice
        ...
        print(f"📊 Count consultants: {count_time:.3f}s ({count_consultants} consultants)")
        ...
        print(f"📋 Count missions: {mission_count_time:.3f}s ({count_missions} missions)")
        
        # Test requête avec JOIN
        start_time = time.time()
        consultants_with_practice = session.query(Consultant).join(Practice, Consultant.practice_id == Practice.id, isouter=True).limit(10).all()
