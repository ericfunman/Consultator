...
from database.models import (
    Consultant, Practice, Mission, Competence, ConsultantCompetence,
    BusinessManager, ConsultantBusinessManager, CV
)
...
CLIENTS = [
    ...
    "AXA", "Allianz", "Generali", "MAIF", "Groupama",
    "Orange", "SFR", "Bouygues Telecom", "Free",
    "EDF", "Engie", "TotalEnergies", "Schneider Electric",
    "Airbus", "Thales", "Dassault Systèmes", "Capgemini",
    "Atos", "Sopra Steria", "Worldline", "Devoteam",
    "SNCF", "Air France-KLM", "Accor", "Carrefour",
    "L'Oréal", "Danone", "Michelin", "Peugeot PSA",
    "Ministère de l'Intérieur", "Ministère des Finances", "CNAM", "Pôle Emploi",
    "La Poste", "RATP", "Veolia", "Vinci"
]
    ...
    for i in range(nb_consultants):
        ...
        if random.random() < 0.9:  # 90% des consultants ont un BM
            ...
            cbm = ConsultantBusinessManager(
                ...
                date_fin=None if random.random() < 0.8 else fake.date_between(start_date=date_debut, end_date='now'),
                commentaire=fake.text(max_nb_chars=100) if random.random() < 0.2 else None
            )
            session.add(cbm)
    ...
    for i, consultant in enumerate(consultants):
        ...
        for competence in consultant_competences:
            cc = ConsultantCompetence(
                ...
                projets_realises=fake.text(max_nb_chars=100) if random.random() < 0.3 else None,
    ...
    for i, consultant in enumerate(consultants):
        ...
        for _ in range(nb_missions):
            ...
            mission = Mission(
                ...
                technologies_utilisees=', '.join(random.sample([comp[0] for comp in COMPETENCES_TECHNIQUES], random.randint(2, 6))),
...
def generate_test_data():
    ...
    with get_database_session() as session:
        try:
            ...
            consultants = create_consultants(session, practices, business_managers, 1000)
            session.commit()
