...
from app.database.models import (
    Consultant, Practice, BusinessManager, Competence, 
    ConsultantCompetence, Mission, Langue, ConsultantLangue,
...
PRACTICES = [
    "Data Science", "Frontend", "Backend", "DevOps", "Mobile", 
    "Cloud", "IA/ML", "Cybersécurité", "Blockchain", "IoT"
]

BUSINESS_MANAGERS = [
    "Marie Dubois", "Pierre Martin", "Sophie Bernard", "Jean Durand",
    "Claire Moreau", "Philippe Leroy", "Nathalie Rousseau", "Marc Petit"
]

COMPETENCES_TECH = [
    "Python", "JavaScript", "Java", "React", "Angular", "Vue.js",
    "Node.js", "Django", "Flask", "Spring Boot", "Docker", "Kubernetes",
    "AWS", "Azure", "GCP", "MySQL", "PostgreSQL", "MongoDB", "Redis",
    "Git", "Jenkins", "GitLab", "Terraform", "Ansible"
]

COMPETENCES_FONCTIONNELLES = [
    "Gestion de projet", "Architecture logicielle", "UX/UI Design",
    "Analyse fonctionnelle", "Tests automatisés", "Agilité", "Scrum",
    "DevOps", "Sécurité", "Performance", "Big Data", "Machine Learning"
]

LANGUES = [
    ("Français", "Natif"), ("Anglais", "Courant"), ("Espagnol", "Intermédiaire"),
    ("Allemand", "Notions"), ("Italien", "Notions"), ("Mandarin", "Débutant")
]

GRADES = ["Junior", "Confirmé", "Senior", "Expert", "Principal", "Architecte"]

CLIENTS = [
    "BNP Paribas", "Société Générale", "Crédit Agricole", "Orange", "SFR",
    "Bouygues", "SNCF", "Air France", "Renault", "Peugeot", "L'Oréal",
    "Danone", "Total", "EDF", "Airbus", "Thales", "Capgemini", "Accenture"
]
...
def create_practices():
    ...
    with get_database_session() as session:
        for i, practice_name in enumerate(PRACTICES):
            existing = session.query(Practice).filter(Practice.nom == practice_name).first()
            if not existing:
                practice = Practice(
                    nom=practice_name,
        ...
        print(f"✅ {len(practices)} nouvelles practices créées ({len(all_practices)} total)")
...
def create_business_managers():
    ...
    with get_database_session() as session:
        for manager_name in BUSINESS_MANAGERS:
            ...
            existing = session.query(BusinessManager).filter(BusinessManager.email == email).first()
            if not existing:
        ...
        print(f"✅ {len(managers)} nouveaux business managers créés ({len(all_managers)} total)")
    return all_managers
...
def create_competences():
    ...
    with get_database_session() as session:
        ...
        for comp_name in COMPETENCES_TECH:
            existing = session.query(Competence).filter(Competence.nom == comp_name).first()
            if not existing:
        ...
        with get_database_session() as session:
            ...
            for i in range(min(batch_size, NB_CONSULTANTS - batch)):
                ...
                consultant = Consultant(
                    ...
                    email=email,
                    telephone=fake.phone_number(),
                    date_entree_societe=fake.date_between(start_date='-10y', end_date='today'),
                    grade=grade,
                ...
                consultant_manager = ConsultantBusinessManager(
                    ...
                    date_debut=consultant.date_entree_societe or fake.date_between(start_date='-5y', end_date='today')
                )
                ...
                for comp_id in consultant_competences:
                    consultant_comp = ConsultantCompetence(
                        ...
                        niveau_maitrise=random.choice(["debutant", "intermediaire", "avance", "expert"]),
            ...
            print(f"✅ Batch {batch//batch_size + 1}: {len(batch_consultants)} consultants créés (Total: {consultants_created})")
