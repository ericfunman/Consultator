#!/usr/bin/env python3
"""
Script de nettoyage automatique des imports inutilisés - Version Projet
Nettoie automatiquement les imports inutilisés dans le projet uniquement
"""
...
class ProjectImportCleaner:
    """Classe principale pour nettoyer les imports du projet"""

    def __init__(self, project_root: str, backup: bool = True):
        self.project_root = Path(project_root)
        self.backup = backup
        self.backup_dir = self.project_root / "backups" / "imports_cleaning_project"
        self.stats = {
            "files_analyzed": 0,
            "files_modified": 0,
            "imports_removed": 0,
    ...
    def find_project_files(self) -> List[Path]:
        """Trouve tous les fichiers Python du projet (excluant venv et dépendances)"""
        python_files = []
        ...
        for root, dirs, files in os.walk(self.project_root):
            ...
            for file in files:
                if file.endswith(".py"):
                    ...
                    # Vérifier si c'est un fichier du projet (pas une dépendance)
    ...
    def _is_project_file(self, file_path: Path) -> bool:
        """Détermine si un fichier appartient au projet (pas aux dépendances)"""
        # Chemins du projet
        project_paths = [
            "app/",
            "config/",
        ...
        # Vérifier si le fichier est dans un chemin du projet
        for project_path in project_paths:
            if project_path in file_str:
                return True

        # Exclure les fichiers qui semblent être des dépendances
        if "site-packages" in file_str or "Scripts" in file_str:
            return False
    ...
    def clean_file(self, analysis: FileAnalysis, dry_run: bool = True) -> bool:
        ...
        try:
            ...
            if dry_run:
                logger.info(
                    f"[DRY RUN] {analysis.file_path}: {len(lines_to_remove)} lignes à supprimer"
                )
                for imp in analysis.unused_imports:
                    logger.info(
                        f"  - Import inutilisé: {imp.module}.{imp.name}"
                ...
                logger.info(
                    f"✅ {analysis.file_path}: {len(lines_to_remove)} lignes supprimées"
                )
    ...
    def run_analysis(self, dry_run: bool = True) -> Dict[str, FileAnalysis]:
        ...
        logger.info("🔍 Démarrage de l'analyse des imports du projet...")
        logger.info(f"📁 Projet: {self.project_root}")

        python_files = self.find_project_files()
        logger.info(f"📄 {len(python_files)} fichiers Python du projet trouvés")
        ...
        for file_path in python_files:
            ...
            if analysis.unused_imports:
                logger.info(
                    f"⚠️  {file_path}: {len(analysis.unused_imports)} imports inutilisés"
                )

        return results

    def run_cleaning(
        self, results: Dict[str, FileAnalysis], dry_run: bool = True
    ) -> bool:
        """Lance le nettoyage basé sur les résultats d'analyse"""
        logger.info(
            "🧹 Démarrage du nettoyage..."
            if not dry_run
            else "🧹 Simulation du nettoyage (DRY RUN)..."
        )

        total_unused = sum(
            len(analysis.unused_imports) for analysis in results.values()
        )

        if total_unused == 0:
            logger.info("✅ Aucun import inutilisé trouvé dans le projet !")
            return True

        logger.info(
            f"📊 {total_unused} imports inutilisés trouvés dans {len(results)} fichiers du projet"
        )
    ...
    def print_summary(self):
        ...
        logger.info("\n" + "=" * 60)
        logger.info("📊 RÉSUMÉ DU NETTOYAGE DES IMPORTS - PROJET")
        logger.info("=" * 60)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Nettoyeur automatique d'imports - Version Projet"
    )
    parser.add_argument(
        "--project-root",
        default=".",
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Appliquer réellement les changements (désactive le dry-run)",
    )
    parser.add_argument(
        "--no-backup", action="store_true", help="Ne pas créer de sauvegardes"
    )
    ...
    # Mode dry-run par défaut, sauf si --apply est spécifié
    dry_run = not args.apply

    # Initialisation
    cleaner = ProjectImportCleaner(args.project_root, backup=not args.no_backup)

    # Analyse
    results = cleaner.run_analysis()

    # Nettoyage
    success = cleaner.run_cleaning(results, dry_run=dry_run)

    # Résumé
    cleaner.print_summary()

    if dry_run:
        logger.info("\n💡 Utilisez --apply pour appliquer les changements")
