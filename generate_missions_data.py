...
def generate_missions_for_consultant(consultant_id: int, consultant_name: str) -> List[Dict]:
...
def create_missions():
    ...
    with get_database_session() as session:
        ...
        if existing_missions > 0:
            ...
            overwrite = input("Voulez-vous supprimer les missions existantes et en créer de nouvelles ? (y/N): ")
        ...
        missions_terminees = session.query(Mission).filter(Mission.statut == "terminee").count()
        total_revenus = session.query(Mission.revenus_generes).filter(Mission.revenus_generes.isnot(None)).all()
...
def main():
    ...
    try:
        ...
        print("Vous pouvez maintenant tester les fonctionnalités de missions dans l'application.")
