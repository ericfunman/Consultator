...
def init_competences_fonctionnelles():
    ...
    try:
        ...
        existing_func = session.query(Competence).filter(
        ...
        for categorie, competences_list in COMPETENCES_FONCTIONNELLES.items():
            ...
            for competence_nom in competences_list:
                ...
                existing = session.query(Competence).filter(
        ...
        # Vérification finale
        total_fonctionnelles = session.query(Competence).filter(
            Competence.type_competence == 'fonctionnelle'
        ).count()
        ...
        for categorie in COMPETENCES_FONCTIONNELLES.keys():
            count = session.query(Competence).filter(
...
def show_competences_summary():
    ...
    try:
        ...
        tech_count = session.query(Competence).filter(
            Competence.type_competence == 'technique'
        ).count()
        print(f"🔧 Compétences techniques: {tech_count}")
        
        # Compétences fonctionnelles
        func_count = session.query(Competence).filter(
            Competence.type_competence == 'fonctionnelle'
        ).count()
        ...
        categories_func = session.query(Competence.categorie).filter(
            Competence.type_competence == 'fonctionnelle'
        ).distinct().all()
        ...
        for (categorie,) in categories_func:
            ...
            exemples = session.query(Competence.nom).filter(
                Competence.categorie == categorie,
                Competence.type_competence == 'fonctionnelle'
            ).limit(3).all()
