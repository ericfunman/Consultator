...
def show():
    ...
    tab1, tab2 = st.tabs(
        ["ðŸ“‹ Liste des consultants", "âž• Ajouter un consultant"]
    )
    ...
    with st.expander("ðŸ§ª Tester l'analyseur de CV", expanded=False):
        ...
        if st.button("ðŸš€ Lancer le test d'analyse", key="test_analyzer"):
            with st.spinner("ðŸ”„ Test de l'analyseur en cours..."):
                ...
                preview_text = DocumentAnalyzer.get_analysis_preview(
                    test_result
                )
        ...
        if custom_text and st.button(
            "ðŸ” Analyser ce texte", key="test_custom"
        ):
            with st.spinner("ðŸ”„ Analyse du texte personnalisÃ©..."):

                custom_result = DocumentAnalyzer.analyze_cv_content(
                    custom_text, "Test PersonnalisÃ©"
                )
...
def show_cv_import():
    ...
    if consultants:
        ...
        st.info(
            f"ðŸ“‹ CV sera associÃ© Ã : **{selected_consultant.prenom} {selected_consultant.nom}**"
        )

        # Afficher le profil existant du consultant
        with st.expander(
            f"ðŸ‘¤ Profil actuel de {selected_consultant.prenom} {selected_consultant.nom}",
            expanded=False,
        ):
            ...
            with col1:
                ...
                if selected_consultant.telephone:
                    st.write(
                        f"**TÃ©lÃ©phone:** {selected_consultant.telephone}"
                    )
                if selected_consultant.salaire_actuel:
                    st.write(
                        f"**Salaire actuel:** {selected_consultant.salaire_actuel:,.0f} â‚¬"
                    )
                st.write(
                    f"**Disponible:** {'âœ… Oui' if selected_consultant.disponibilite else 'âŒ Non'}"
                )
                ...
                try:
                    ...
                    with get_database_session() as session:
                        missions = (
                            ...
                            .filter(
                                Mission.consultant_id == selected_consultant.id
                            )
                            .all()
                        )
                        ...
                        if missions:
                            for mission in missions[
                                ...
                                st.write(
                                    f"{status_icon} **{mission.client}** ({mission.date_debut} â†’ {mission.date_fin or 'En cours'})"
                                )
                                if mission.technologies_utilisees:
                                    st.caption(
                                        f"Technologies: {mission.technologies_utilisees}"
                                    )

                            if len(missions) > 5:
                                st.caption(
                                    f"... et {len(missions) - 5} autres missions"
                                )
                        else:
                ...
                try:
                    ...
                    with get_database_session() as session:
                        competences = (
                            ...
                            .join(Competence)
                            .filter(
                                ConsultantCompetence.consultant_id
                        ...
                        tech_competences = [
                            (cc, comp)
                            for cc, comp in competences
                            if comp.type_competence == "technique"
                        ]
                        if tech_competences:
                            for cc, comp in tech_competences[:10]:  # Top 10
                                niveau_icon = (
                                    "ðŸŸ¢"
                                    if cc.niveau_maitrise == "expert"
                                    else (
                                        "ðŸŸ¡"
                                        if cc.niveau_maitrise
                                        == "intermediaire"
                                        else "ðŸ”´"
                                    )
                                )
                                st.write(
                                    f"{niveau_icon} **{comp.nom}** ({cc.annees_experience:.1f} ans)"
                                )

                            if len(tech_competences) > 10:
                                st.caption(
                                    f"... et {len(tech_competences) - 10} autres compÃ©tences techniques"
                                )
                        else:
                            st.write(
                                "Aucune compÃ©tence technique enregistrÃ©e"
                            )
                        ...
                        func_competences = [
                            (cc, comp)
                            for cc, comp in competences
                            if comp.type_competence == "fonctionnelle"
                        ]
                        if func_competences:
                            ...
                            if len(func_competences) > 5:
                                st.caption(
                                    f"... et {len(func_competences) - 5} autres compÃ©tences fonctionnelles"
                                )
                        else:
                            st.write(
                                "Aucune compÃ©tence fonctionnelle enregistrÃ©e"
                            )

                except Exception as e:
                    st.error(
                        f"Erreur lors du chargement des compÃ©tences: {e}"
                    )

            st.info(
                "ðŸ’¡ **Note**: L'analyse du CV ajoutera de nouvelles missions et compÃ©tences Ã  ce profil existant. Les doublons seront automatiquement Ã©vitÃ©s."
            )
        ...
        if uploaded_file is not None:
            ...
            if st.button(
                ...
                with st.spinner("ðŸ”„ Sauvegarde en cours..."):
                    ...
                    consultant_dir = (
                        f"data/uploads/consultant_{selected_consultant.id}"
                    )
                    ...
                    try:
                        ...
                        st.info(
                            "ðŸ” Analyse du contenu - FonctionnalitÃ© Ã  venir"
                        )
            ...
            if os.path.exists(consultant_dir):
                files = [
                    f
                    for f in os.listdir(consultant_dir)
                    if os.path.isfile(os.path.join(consultant_dir, f))
                ]
                    ...
                    for file in files:
                        ...
                        with st.container():
                            ...
                            with col1:
                                ...
                                st.caption(
                                    f"ðŸ“… {file_modified.strftime('%d/%m/%Y %H:%M')} â€¢ ðŸ’¾ {file_size_mb:.2f} MB"
                                )
                            ...
                            with col3:
                                ...
                                if st.button(
                                    "ðŸ‘ï¸",
                                    key=f"view_{file}",
                                    help="AperÃ§u du fichier",
                                ):
                                    st.session_state[
                                        f"show_preview_{file}"
                                    ] = True
                            ...
                            with col4:
                                ...
                                if st.button(
                                    ...
                                    st.session_state[f"show_info_{file}"] = (
                                        True
                                    )
                            ...
                            with col5:
                                ...
                                if st.button(
                                    ...
                                    with st.spinner(
                                        "ðŸ”„ Analyse du CV en cours..."
                                    ):

                                        # Extraire le texte
                                        text = DocumentAnalyzer.extract_text_from_file(
                                            file_path
                                        )
                                        ...
                                        if text.strip():
                                            ...
                                            st.session_state[
                                                f"show_analysis_{file}"
                                            ] = True

                                            st.success(
                                                "âœ… Analyse terminÃ©e ! VÃ©rifiez les rÃ©sultats ci-dessous."
                                            )
                                            st.rerun()
                                        else:
                                            st.error(
                                                "âŒ Impossible d'extraire le texte du fichier"
                                            )
                        ...
                        if st.session_state.get(f"show_preview_{file}", False):
                            with st.expander(
                                f"ðŸ‘ï¸ AperÃ§u de {file}", expanded=True
                            ):
                                try:
                                    if file.lower().endswith(".pdf"):
                                        st.info(
                                            "ðŸ“„ Fichier PDF - AperÃ§u non disponible dans cette version"
                                        )
                                        st.markdown(
                                            f"**Chemin complet:** `{file_path}`"
                                        )

                                        # Option pour ouvrir avec l'application par dÃ©faut
                                        if st.button(
                                            f"ðŸš€ Ouvrir avec l'application par dÃ©faut",
                                            key=f"open_{file}",
                                        ):
                                            try:
                                                import platform
                                                ...
                                                st.success(
                                                    f"ðŸš€ Ouverture de {file}"
                                                )
                                            except Exception as e:
                                                st.error(
                                                    f"âŒ Impossible d'ouvrir le fichier: {e}"
                                                )
                                        ...
                                        st.info(
                                            "ðŸ“ Fichier Word - AperÃ§u non disponible dans cette version"
                                        )
                                        st.markdown(
                                            f"**Chemin complet:** `{file_path}`"
                                        )
                                        ...
                                        st.info(
                                            "ðŸ“Š Fichier PowerPoint - AperÃ§u non disponible dans cette version"
                                        )
                                        st.markdown(
                                            f"**Chemin complet:** `{file_path}`"
                                        )
                                        ...
                                        st.warning(
                                            "â“ Type de fichier non reconnu pour l'aperÃ§u"
                                        )

                                except Exception as e:
                                    st.error(
                                        f"âŒ Erreur lors de l'aperÃ§u: {e}"
                                    )

                                # Bouton pour fermer l'aperÃ§u
                                if st.button(
                                    "âŒ Fermer l'aperÃ§u",
                                    key=f"close_preview_{file}",
                                ):
                        ...
                        if st.session_state.get(f"show_info_{file}", False):
                            with st.expander(
                                f"â„¹ï¸ Informations dÃ©taillÃ©es - {file}",
                                expanded=True,
                            ):
                                try:
                                    ...
                                    info_data = {
                                        ...
                                        "ðŸ“… Date de crÃ©ation": datetime.fromtimestamp(
                                            file_stat.st_ctime
                                        ).strftime(
                                            "%d/%m/%Y %H:%M:%S"
                                        ),
                                        "ðŸ”„ DerniÃ¨re modification": file_modified.strftime(
                                            "%d/%m/%Y %H:%M:%S"
                                        ),
                                    ...
                                    st.error(
                                        f"âŒ Erreur lors de la rÃ©cupÃ©ration des informations: {e}"
                                    )

                                # Bouton pour fermer les informations
                                if st.button(
                                    "âŒ Fermer les informations",
                                    key=f"close_info_{file}",
                                ):
                        ...
                        if st.session_state.get(
                            ...
                            with st.expander(
                                f"ðŸ” Analyse du CV - {file}", expanded=True
                            ):
                                if analysis_data:
                                    # Afficher l'aperÃ§u de l'analyse
                                    preview_text = (
                                        DocumentAnalyzer.get_analysis_preview(
                                            analysis_data
                                        )
                                    )
                                    st.markdown(preview_text)
                                    ...
                                    st.subheader(
                                        "âœ… Validation et modification"
                                    )
                                    st.info(
                                        "ðŸ”§ Vous pouvez modifier les donnÃ©es dÃ©tectÃ©es avant de les sauvegarder"
                                    )

                                    # Formulaire d'Ã©dition des missions
                                    missions = analysis_data.get(
                                        "missions", []
                                    )
                                    if missions:
                                        st.markdown(
                                            "### ðŸš€ Missions dÃ©tectÃ©es"
                                        )
                                    ...
                                    st.markdown(
                                        "### ðŸŽ¯ CompÃ©tences fonctionnelles"
                                    )
                                    current_func = ", ".join(
                                        analysis_data.get(
                                    ...
                                    with col1:
                                        if st.button(
                                            "ðŸ’¾ Sauvegarder dans le profil",
                                            key=f"save_analysis_{file}",
                                            type="primary",
                                        ):
                                            # PrÃ©parer les donnÃ©es finales
                                            final_missions = [
                                                m
                                                for m in edited_missions
                                                if m["client"].strip()
                                            ]
                                            ...
                                            for (
                                                tech
                                            ) in edited_global_techs.replace(
                                                "\n", ","
                                            ).split(
                                                ","
                                            ):
                                            ...
                                            try:
                                                success = ConsultantService.save_cv_analysis(
                                                    selected_consultant.id,
                                                    final_analysis,
                                                )
                                                ...
                                                if success:
                                                    st.success(
                                                        f"âœ… Analyse sauvegardÃ©e avec succÃ¨s dans le profil de {selected_consultant.prenom} {selected_consultant.nom} !"
                                                    )
                                                    st.balloons()

                                                    # Afficher un rÃ©sumÃ© de ce qui a Ã©tÃ© sauvegardÃ©
                                                    with st.expander(
                                                        "ðŸ“‹ RÃ©sumÃ© des donnÃ©es sauvegardÃ©es",
                                                        expanded=True,
                                                    ):
                                                        st.write(
                                                            f"**ðŸ‘¤ Consultant:** {selected_consultant.prenom} {selected_consultant.nom}"
                                                        )
                                                        st.write(
                                                            f"**ðŸš€ Missions ajoutÃ©es:** {len(final_missions)}"
                                                        )
                                                        for (
                                                            mission
                                                        ) in final_missions:
                                                            st.write(
                                                                f"  â€¢ {mission['client']} ({mission['date_debut']} â†’ {mission['date_fin']})"
                                                            )
                                                        st.write(
                                                            f"**ðŸ’» Technologies ajoutÃ©es:** {len(global_techs)}"
                                                        )
                                                        if global_techs:
                                                            st.write(
                                                                f"  {', '.join(global_techs[:10])}{'...' if len(global_techs) > 10 else ''}"
                                                            )
                                                        st.write(
                                                            f"**ðŸŽ¯ CompÃ©tences fonctionnelles ajoutÃ©es:** {len(func_skills)}"
                                                        )
                                                        if func_skills:
                                                            st.write(
                                                                f"  {', '.join(func_skills)}"
                                                        ...
                                                        st.info(
                                                            "ðŸ’¡ **Note:** Les compÃ©tences et missions en doublon ont Ã©tÃ© automatiquement ignorÃ©es."
                                                        )

                                                        # Proposer de voir le profil mis Ã  jour
                                                        if st.button(
                                                            "ðŸ‘€ Voir le profil mis Ã  jour",
                                                            key=f"view_profile_{file}",
                                                        ):
                                                            st.session_state.view_consultant_id = (
                                                                selected_consultant.id
                                                            )
                                                            st.rerun()
                                                else:
                                                    st.error(
                                                        "âŒ Erreur lors de la sauvegarde. VÃ©rifiez les logs pour plus de dÃ©tails."
                                                    )

                                            except Exception as e:
                                                st.error(
                                                    f"âŒ Erreur lors de la sauvegarde: {e}"
                                                )
                                                st.exception(e)
                                    ...
                                    with col2:
                                        if st.button(
                                            "ðŸ”„ Re-analyser",
                                            key=f"reanalyze_{file}",
                                        ):
                                            # Relancer l'analyse
                                            with st.spinner(
                                                "ðŸ”„ Nouvelle analyse en cours..."
                                            ):
                                                ...
                                                if text.strip():
                                                    ...
                                                    st.success(
                                                        "âœ… Nouvelle analyse terminÃ©e !"
                                                    )
                                                    st.rerun()

                                    with col3:
                                        if st.button(
                                            "âŒ Fermer l'analyse",
                                            key=f"close_analysis_{file}",
                                        ):
                                    ...
                                    st.error(
                                        "âŒ Aucune donnÃ©e d'analyse disponible"
                                    )

                        st.divider()
    else:
        st.warning(
            "âš ï¸ Aucun consultant disponible. Ajoutez d'abord des consultants."
        )
...
def show_consultant_profile():
    ...
    with col1:
        st.metric(
            "ðŸ’° Salaire annuel",
            f"{consultant.salaire_actuel or 0:,}â‚¬",
            delta=None,
        )
...
def show_consultant_info(consultant):
    ...
    with st.form(f"edit_consultant_info_{consultant.id}"):
        ...
        with col2:
            nom = st.text_input(
                "ðŸ‘¤ Nom *", value=consultant.nom, placeholder="Ex: Dupont"
            )
            ...
            disponibilite = st.checkbox(
                "âœ… Disponible", value=consultant.disponibilite
            )
        ...
        if submitted:
            ...
            if not prenom or not nom or not email:
                st.error(
                    "âŒ Veuillez remplir tous les champs obligatoires (*)"
                )
            else:
                ...
                if (
                    ...
                    st.error(
                        f"âŒ Un consultant avec l'email {email} existe dÃ©jÃ  !"
                    )
                else:
                    try:
                        ...
                        st.success(
                            f"âœ… {prenom} {nom} a Ã©tÃ© modifiÃ© avec succÃ¨s !"
                        )
    ...
    with col1:
        st.warning(
            "âš ï¸ **Attention**: La suppression du consultant est irrÃ©versible et effacera toutes ses donnÃ©es."
        )

    with col2:
        if st.button(
            "ðŸ—‘ï¸ Supprimer le consultant",
            type="secondary",
            use_container_width=True,
    ...
    if st.session_state.get("show_delete_confirmation", False):
        with st.container():
            ...
            st.write(
                f"Voulez-vous vraiment supprimer **{consultant.prenom} {consultant.nom}** ?"
            )
            ...
            with col1:
                if st.button(
                    "âœ… Oui, supprimer dÃ©finitivement",
                    type="primary",
                    use_container_width=True,
                    key="confirm_delete",
                ):
                    try:
                        ...
                        st.success(
                            f"âœ… {consultant.prenom} {consultant.nom} a Ã©tÃ© supprimÃ© avec succÃ¨s !"
                        )
    ...
    with col1:
        ...
        creation_date = (
            consultant.date_creation.strftime("%d/%m/%Y Ã  %H:%M")
            if consultant.date_creation
            else "N/A"
        )
    ...
    with col2:
        ...
        maj_date = (
            consultant.derniere_maj.strftime("%d/%m/%Y Ã  %H:%M")
            if consultant.derniere_maj
            else "N/A"
        )
        st.info(maj_date)
    ...
    skills_data = [
        {
            ...
            "DerniÃ¨re utilisation": "2024",
        },
        {
            "CompÃ©tence": "Machine Learning",
            "Niveau": "AvancÃ©",
            "AnnÃ©es d'expÃ©rience": 3,
            "DerniÃ¨re utilisation": "2024",
        },
        {
            "CompÃ©tence": "SQL",
            "Niveau": "Expert",
            "AnnÃ©es d'expÃ©rience": 4,
            "DerniÃ¨re utilisation": "2024",
        },
        {
            "CompÃ©tence": "Docker",
            "Niveau": "IntermÃ©diaire",
            "AnnÃ©es d'expÃ©rience": 2,
            "DerniÃ¨re utilisation": "2023",
        },
    ]
    ...
    if skills_data:
        ...
        with col1:
            if st.button("âž• Ajouter une compÃ©tence"):
                st.info(
                    "ðŸ”§ FonctionnalitÃ© Ã  venir - Redirection vers gestion des compÃ©tences"
                )

        with col2:
            if st.button("âœï¸ Modifier les compÃ©tences"):
                st.info(
                    "ðŸ”§ FonctionnalitÃ© Ã  venir - Ã‰dition des compÃ©tences"
                )

        with col3:
            if st.button("ðŸ“Š Analyse des compÃ©tences"):
                st.info(
                    "ðŸ”§ FonctionnalitÃ© Ã  venir - Analytics des compÃ©tences"
                )
...
def show_consultant_missions(consultant):
    ...
    # Onglets pour organiser les fonctionnalitÃ©s
    tab1, tab2 = st.tabs(
        ["ðŸ“‹ Missions existantes", "âž• Ajouter une mission"]
    )
...
def show_existing_missions(consultant):
    ...
    if missions_db:
        ...
        if st.checkbox("âœï¸ Mode Ã©dition", key="edit_mode"):
            st.info(
                "ðŸ“ Mode Ã©dition activÃ© - Vous pouvez maintenant modifier les missions"
            )

            for i, mission in enumerate(missions_db):
                with st.expander(
                    f"âœï¸ Ã‰diter: {mission.client} - {mission.role or 'RÃ´le non dÃ©fini'}",
                    expanded=False,
                ):
            ...
            for i, mission in enumerate(missions_db):
                with st.expander(
                    f"ðŸš€ {mission.client} - {mission.role or 'RÃ´le non dÃ©fini'}",
                    expanded=(i == 0),
                ):
        ...
        st.markdown(
            "- Utiliser l'onglet **'Ajouter une mission'** pour crÃ©er une mission manuellement"
        )
        st.markdown(
            "- Importer un CV dans l'onglet **'Import CV'** pour dÃ©tecter automatiquement les missions"
        )
...
def show_mission_details(mission, index):
    ...
    with col1:
        ...
        st.write(
            f"**ðŸ“… DÃ©but**: {mission.date_debut.strftime('%Y-%m-%d') if mission.date_debut else 'N/A'}"
        )
        st.write(f"**ðŸ’° Revenus**: {mission.revenus_generes or 0:,}â‚¬")

    with col2:
        st.write(
            f"**ðŸ“… Fin**: {mission.date_fin.strftime('%Y-%m-%d') if mission.date_fin else 'En cours'}"
        )
    ...
    st.write(
        f"**ðŸ› ï¸ Technologies**: {mission.technologies_utilisees or 'Non spÃ©cifiÃ©es'}"
    )
    ...
    with st.form(f"edit_mission_form_{key_prefix}"):
        ...
        with col1:
            if st.form_submit_button(
                "ï¿½ Sauvegarder les modifications", type="primary"
            ):
        ...
        with col2:
            if st.form_submit_button(
                "ðŸ—‘ï¸ Supprimer la mission", type="secondary"
            ):
...
def show_add_mission_form(consultant):
    ...
    with st.form("add_mission_form"):
        ...
        with col1:
            ...
            client = st.text_input(
                "ðŸ¢ Client", placeholder="Ex: SociÃ©tÃ© GÃ©nÃ©rale"
            )
            role = st.text_input(
                "ðŸ‘¤ RÃ´le", placeholder="Ex: Lead Developer"
            )
            revenus = st.number_input(
                "ï¿½ Revenus (â‚¬)", min_value=0.0, value=0.0
            )
        ...
        with col2:
            ...
            statut = st.selectbox(
                "ï¿½ Statut", ["en_cours", "terminee", "en_pause"]
            )
...
def show_consultant_documents(consultant):
            ...
            real_docs.append(
                {
                    "Type": (
                        "CV"
                        if any(ext in file.lower() for ext in [".pdf", ".doc"])
                        else "Document"
                    ),
                    "Nom": file,
    ...
    if all_docs:
        for doc in all_docs:
            with st.container():
                ...
                with col3:
                    ...
                    if doc.get("Path"):
                        ...
                        if st.button(
                            "ðŸ“¥",
                            key=f"download_sim_{doc['Nom']}",
                            help="Document simulÃ©",
                        ):
                ...
                with col4:
                    ...
                    if doc.get("Path"):
                        if st.button(
                            "ðŸ‘ï¸",
                            key=f"view_profile_{doc['Nom']}",
                            help="AperÃ§u du fichier",
                        ):
                            st.session_state[
                                f"show_profile_preview_{doc['Nom']}"
                            ] = True
                    else:
                        if st.button(
                            "ðŸ‘ï¸",
                            key=f"view_sim_{doc['Nom']}",
                            help="AperÃ§u simulÃ©",
                        ):
                ...
                with col5:
                    ...
                    if doc.get("Path"):
                        if st.button(
                            "â„¹ï¸",
                            key=f"info_profile_{doc['Nom']}",
                            help="Informations dÃ©taillÃ©es",
                        ):
                            st.session_state[
                                f"show_profile_info_{doc['Nom']}"
                            ] = True
                    else:
                        if st.button(
                            "â„¹ï¸",
                            key=f"info_sim_{doc['Nom']}",
                            help="Informations simulÃ©es",
                        ):
                            st.info(
                                f"ðŸ”§ Informations simulÃ©es - {doc['Nom']}"
                            )
                ...
                with col6:
                    ...
                    if doc.get("Path"):  # Fichier rÃ©el
                        ...
                        if st.button(
                            "ï¿½ï¸",
                            key=f"delete_sim_{doc['Nom']}",
                            help="Document simulÃ©",
                        ):
                            st.info(f"ðŸ”§ Document simulÃ© - {doc['Nom']}")

            # Affichage conditionnel de l'aperÃ§u (fichiers rÃ©els seulement)
            if doc.get("Path") and st.session_state.get(
                f"show_profile_preview_{doc['Nom']}", False
            ):
                with st.expander(
                    f"ðŸ‘ï¸ AperÃ§u de {doc['Nom']}", expanded=True
                ):
                    try:
                        file_path = doc["Path"]
                        file_name = doc["Nom"]
                        ...
                        if file_name.lower().endswith(".pdf"):
                            ...
                            if st.button(
                                f"ðŸš€ Ouvrir avec l'application par dÃ©faut",
                                key=f"open_profile_{file_name}",
                            ):
                                try:
                                    os.startfile(file_path)
                                    st.success(
                                        f"ðŸš€ Ouverture de {file_name}"
                                    )
                                except Exception as e:
                                    st.error(
                                        f"âŒ Impossible d'ouvrir le fichier: {e}"
                    ...
                    if st.button(
                        "âŒ Fermer l'aperÃ§u",
                        key=f"close_profile_preview_{doc['Nom']}",
                    ):
                        del st.session_state[
                            f"show_profile_preview_{doc['Nom']}"
                        ]
            ...
            if doc.get("Path") and st.session_state.get(
                f"show_profile_info_{doc['Nom']}", False
            ):
                with st.expander(
                    f"â„¹ï¸ Informations dÃ©taillÃ©es - {doc['Nom']}",
                    expanded=True,
                ):
                    try:
                        file_path = doc["Path"]
                        ...
                        info_data = {
                            ...
                            "ï¿½ Date de crÃ©ation": datetime.fromtimestamp(
                                file_stat.st_ctime
                            ).strftime("%d/%m/%Y %H:%M:%S"),
                            "ðŸ”„ DerniÃ¨re modification": datetime.fromtimestamp(
                                file_stat.st_mtime
                            ).strftime(
                                "%d/%m/%Y %H:%M:%S"
                            ),
                    ...
                    if st.button(
                        "âŒ Fermer les informations",
                        key=f"close_profile_info_{doc['Nom']}",
                    ):
        ...
        with col1:
            st.info(
                "ðŸ“Ž Utilisez la section 'Import CV' ci-dessous pour ajouter de vrais documents"
            )
        ...
        st.info(
            "ðŸ’¡ Utilisez la section 'Import CV' pour ajouter des documents"
        )
...
def show_consultant_actions(consultant):
    ...
    with action_tab1:
        ...
        st.info(
            "ðŸ’¡ **Info**: Vous pouvez modifier les informations directement dans l'onglet 'Informations'"
        )
    ...
    with action_tab2:
        ...
        with col2:
            ...
            if st.button(
                "ðŸ“‹ Dupliquer ce profil",
                key="duplicate_profile",
                use_container_width=True,
            ):
                st.info(
                    "ðŸ”§ FonctionnalitÃ© Ã  venir - Duplication de profil"
                )
...
def show_edit_consultant_inline(consultant):
    ...
    with st.form(f"edit_consultant_inline_{consultant.id}"):
        ...
        with col1:
            prenom = st.text_input(
                "PrÃ©nom *", value=consultant.prenom, placeholder="Ex: Jean"
            )
            email = st.text_input(
                "Email *",
                value=consultant.email,
        ...
        with col1:
            submitted = st.form_submit_button(
                "âœ… Sauvegarder", use_container_width=True
            )

        with col2:
            cancelled = st.form_submit_button(
                "âŒ Annuler", use_container_width=True
            )
        ...
        if submitted:
            ...
            # VÃ©rifier l'unicitÃ© de l'email (sauf pour le consultant actuel)
            existing_consultant = ConsultantService.get_consultant_by_email(
                email
            )
...
def show_delete_consultant_inline(consultant):
    ...
    st.warning(
        "âš ï¸ **Attention**: Cette action est irrÃ©versible et supprimera dÃ©finitivement toutes les donnÃ©es du consultant."
    )
    ...
    with st.expander("ðŸ“‹ DonnÃ©es qui seront supprimÃ©es", expanded=False):
        ...
        with col2:
            ...
            st.write(
                f"â€¢ Statut: {'Disponible' if consultant.disponibilite else 'En mission'}"
            )

            # RÃ©cupÃ©rer les statistiques de maniÃ¨re sÃ©curisÃ©e
            consultant_stats = ConsultantService.get_consultant_with_stats(
                consultant.id
            )
            if consultant_stats:
    ...
    with col2:
        if st.button(
            "âŒ Annuler", use_container_width=True, key="delete_cancel"
        ):
    ...
    with col1:
        if st.button(
            "ðŸ“¤ Exporter le profil complet", use_container_width=True
        ):
...
def show_consultants_list():
    ...
    with col1:
        search_name = st.text_input(
            "ðŸ” Rechercher par nom", placeholder="Nom ou prÃ©nom..."
        )

    with col2:
        filter_availability = st.selectbox(
            "ðŸ“Š DisponibilitÃ©", ["Tous", "Disponible", "En mission"]
        )

    with col3:
        sort_by = st.selectbox(
            "ðŸ“ˆ Trier par", ["Nom", "Date d'ajout", "DerniÃ¨re mise Ã  jour"]
        )

    with col4:
        st.write("")  # Espacement
        if st.button(
            "ðŸ”„", help="Actualiser la liste", key="refresh_consultants"
        ):
    ...
    try:
        ...
        if consultants:
                ...
                consultants_data.append(
                    {
                        ...
                        "DerniÃ¨re MAJ": (
                            consultant.derniere_maj.strftime("%Y-%m-%d")
                            if consultant.derniere_maj
                            else "-"
                        ),
                    }
            ...
            if filter_availability != "Tous":
                status = (
                    "âœ… Disponible"
                    if filter_availability == "Disponible"
                    else "ðŸ”´ En mission"
                )
            ...
            if not df.empty:
                st.markdown(
                    "### ðŸ‘¥ Consultants - Cliquez sur une ligne pour voir le profil dÃ©taillÃ©"
                )
                ...
                if event.selection.rows:
                    ...
                    st.session_state.selected_consultant_id = int(
                        selected_consultant["ID"]
                    )
                    ...
                    st.session_state.edit_consultant_id = int(
                        selected_consultant["ID"]
                    )

                    # Afficher automatiquement le profil dÃ©taillÃ©
                    st.session_state.view_consultant_profile = int(
                        selected_consultant["ID"]
                    )
                    st.rerun()
                else:
                    st.info(
                        "ðŸ‘† Cliquez sur une ligne du tableau pour sÃ©lectionner un consultant"
                    )
                ...
                with col2:
                    if st.button("ðŸ“Š GÃ©nÃ©rer rapport"):
                        st.info(
                            "ðŸ“Š GÃ©nÃ©ration de rapport - FonctionnalitÃ© Ã  venir !"
                        )

                with col3:
                    if st.button("âœ‰ï¸ Email groupÃ©"):
                        st.info(
                            "âœ‰ï¸ Email groupÃ© - FonctionnalitÃ© Ã  venir !"
                        )

            else:
                st.info(
                    "Aucun consultant ne correspond aux critÃ¨res de recherche."
                )

        else:
            st.info(
                "ðŸ“ Aucun consultant enregistrÃ©. Utilisez l'onglet 'Ajouter un consultant' pour commencer !"
            )
...
def show_add_consultant_form():
    ...
    form_key = (
        f"add_consultant_form_{st.session_state.get('form_reset_counter', 0)}"
    )
    ...
    with st.form(form_key):
        ...
        with col1:
            ...
            salaire = st.number_input(
                "Salaire annuel (â‚¬)", min_value=0, value=45000, step=1000
            )

        with col2:
            nom = st.text_input("Nom *", placeholder="Ex: Dupont")
            telephone = st.text_input(
                "TÃ©lÃ©phone", placeholder="01.23.45.67.89"
            )
            disponible = st.checkbox("Disponible", value=True)

        notes = st.text_area(
            "Notes", placeholder="Informations complÃ©mentaires..."
        )
        ...
        with col1:
            submitted = st.form_submit_button(
                "âœ… Ajouter le consultant", type="primary"
            )
        ...
        if submitted:
            ...
            if not prenom or not nom or not email:
                st.error(
                    "âŒ Veuillez remplir tous les champs obligatoires (PrÃ©nom, Nom, Email)"
                )
            else:
                ...
                if ConsultantService.create_consultant(consultant_data):
                    st.success(
                        f"âœ… Consultant {prenom} {nom} ajoutÃ© avec succÃ¨s !"
                    )
                    st.balloons()
                    ...
                    st.info(
                        "âž¡ï¸ Redirection automatique vers la liste des consultants..."
                    )
                    ...
                    st.info(
                        "ðŸ”„ Consultant ajoutÃ© ! Allez voir la liste des consultants pour le retrouver."
                    )
                    st.success(
                        "ðŸ“ Le formulaire est maintenant rÃ©initialisÃ© pour un nouvel ajout."
                    )
                    ...
                    st.error(
                        "âŒ Erreur lors de l'ajout du consultant. VÃ©rifiez la base de donnÃ©es."
                    )
...
def show_edit_consultant_form():
    ...
    if st.checkbox(
        "ðŸ” Debug: Voir l'Ã©tat de la session", key="debug_session"
    ):
    ...
    if "edit_consultant_id" in st.session_state:
        ...
        try:
            ...
            st.success(
                f"âœï¸ **Consultant sÃ©lectionnÃ© depuis la liste**: {consultant.prenom} {consultant.nom}"
            )
        ...
        if not consultants:
            st.info(
                "ðŸ“ Aucun consultant Ã  modifier. Ajoutez d'abord des consultants."
            )
            return
        ...
        st.info(
            "ðŸ’¡ **Conseil**: Vous pouvez aussi sÃ©lectionner un consultant dans la 'Liste des consultants' puis cliquer sur 'Modifier'"
        )
    ...
    with st.form("edit_consultant_form"):
        ...
        with col2:
            ...
            telephone = st.text_input(
                "TÃ©lÃ©phone", value=consultant.telephone or ""
            )
        ...
        if submitted:
            ...
            if not prenom or not nom or not email:
                ...
                if ConsultantService.update_consultant(
                    ...
                    st.success(
                        f"âœ… Consultant {prenom} {nom} mis Ã  jour avec succÃ¨s !"
                    )
                    st.balloons()
                    ...
                    st.info(
                        "âž¡ï¸ Retour automatique Ã  la liste des consultants..."
                    )
                    ...
                    st.error(
                        "âŒ Erreur lors de la mise Ã  jour du consultant."
                    )
        ...
        if preview:
            ...
            if telephone != (consultant.telephone or ""):
                changes["TÃ©lÃ©phone"] = (
                    f"{consultant.telephone or 'Vide'} â†’ {telephone or 'Vide'}"
                )
            if salaire != int(consultant.salaire_actuel or 0):
                changes["Salaire"] = (
                    f"{consultant.salaire_actuel or 0}â‚¬ â†’ {salaire}â‚¬"
                )
            if disponible != consultant.disponibilite:
                changes["DisponibilitÃ©"] = (
                    f"{'Oui' if consultant.disponibilite else 'Non'} â†’ {'Oui' if disponible else 'Non'}"
                )
            if notes != (consultant.notes or ""):
                changes["Notes"] = (
                    f"{'ModifiÃ©es' if notes != (consultant.notes or '') else 'InchangÃ©es'}"
                )
...
def show_delete_consultant_form():
    ...
    # VÃ©rifier si un consultant a Ã©tÃ© sÃ©lectionnÃ© depuis la liste
    if (
        "selected_consultant_id" in st.session_state
        and "selected_consultant_name" in st.session_state
    ):
        # RÃ©cupÃ©rer le consultant sÃ©lectionnÃ©
        try:
            ...
            st.success(
                f"ðŸŽ¯ **Consultant sÃ©lectionnÃ© depuis la liste**: {consultant_name}"
            )
            ...
            with col1:
                ...
                st.write(
                    f"ðŸ’° **Salaire**: {consultant.salaire_actuel or 0}â‚¬"
                )
            with col2:
                st.write(
                    f"ðŸ“ž **TÃ©lÃ©phone**: {consultant.telephone or 'Non renseignÃ©'}"
                )
                st.write(
                    f"âœ… **Disponible**: {'Oui' if consultant.disponibilite else 'Non'}"
                )

            st.warning(
                "âš ï¸ Cette action est **IRRÃ‰VERSIBLE** et supprimera toutes les donnÃ©es associÃ©es."
            )
            ...
            with col1:
                if st.button(
                    "âœ… OUI, SUPPRIMER", type="primary", key="confirm_delete"
                ):
                    try:
                        result = ConsultantService.delete_consultant(
                            consultant_id
                        )
                        ...
                        if result:
                            ...
                            st.error(
                                "âŒ Erreur lors de la suppression du consultant."
                            )
                    except Exception as e:
        ...
        **Workflow de suppression simplifiÃ© :**
        ...
        if consultants:
                ...
                consultant_info.append(
                    {
                        ...
                        "Email": consultant.email,
                        "Disponible": (
                            "âœ…" if consultant.disponibilite else "âŒ"
                        ),
                    }
                )
            ...
            st.info(
                f"ðŸ“ˆ **Total**: {len(consultants)} consultant(s) dans la base de donnÃ©es"
            )
        else:
        ...
        st.markdown("---")
        st.success(
            "ðŸ’¡ **Conseil**: SÃ©lectionnez un consultant dans la 'Liste des consultants' puis revenez ici"
        )
