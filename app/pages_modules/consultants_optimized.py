...
try:
    ...
    from database.models import Mission, Competence, ConsultantCompetence, Consultant, BusinessManager, Practice
    from services.consultant_service import ConsultantService
    from config.performance import PAGINATION_CONFIG, CACHE_CONFIG, get_optimal_page_size, is_large_dataset
    imports_ok = True
...
def get_consultants_paginated(page: int = 1, search_term: str = "", per_page: int = 50) -> Dict:
    ...
    try:
        if search_term:
            consultants = ConsultantService.search_consultants_optimized(search_term, page, per_page)
        else:
...
def show_performance_info():
    ...
    if dataset_info.get('is_large', False):
        with st.expander("⚡ Informations de performance", expanded=False):
            ...
            with col3:
                st.metric("⚡ Optimisations", "✅ Activées" if dataset_info.get('optimizations_enabled') else "❌ Désactivées")
...
def show_consultants_list_optimized():
    ...
    with col_filters:
        show_available_only = st.checkbox("✅ Disponibles uniquement", help="Filtrer les consultants disponibles")
    ...
    if total > 0:
        ...
        with col_nav:
            ...
            with col_prev:
                if st.button("⬅️ Précédent", disabled=(st.session_state.consultant_page <= 1)):
                    st.session_state.consultant_page -= 1
                    st.rerun()
            ...
            with col_next:
                if st.button("➡️ Suivant", disabled=(st.session_state.consultant_page >= pages)):
                    st.session_state.consultant_page += 1
                    st.rerun()
    ...
    if consultants:
        ...
        columns_to_show = ['id', 'prenom', 'nom', 'email', 'practice_name', 'disponibilite']
        if all(col in df_display.columns for col in columns_to_show):
            df_filtered = df_display[columns_to_show].copy()
            
            # Renommage des colonnes pour l'affichage
            df_filtered.columns = ['ID', 'Prénom', 'Nom', 'Email', 'Practice', 'Disponible']
...
def show_quick_add_consultant():
    ...
    with st.form("quick_add_consultant", clear_on_submit=True):
        ...
        with col2:
            ...
            salaire = st.number_input("Salaire (€)", min_value=0, value=45000, step=1000)
...
def show_detailed_statistics():
    ...
    with col2:
        ...
        mission_data = {
            ...
            'Total missions': stats.get('total_missions', 0) - stats.get('active_missions', 0)
        }
    ...
    with col_metrics[0]:
        avg_missions = stats.get('total_missions', 0) / max(stats.get('total_consultants', 1), 1)
        st.metric("📊 Missions/Consultant", f"{avg_missions:.1f}")
    
    with col_metrics[1]:
        utilization = (stats.get('busy_consultants', 0) / max(stats.get('total_consultants', 1), 1)) * 100
