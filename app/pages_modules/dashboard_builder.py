...
class DashboardBuilder:
    ...
    def show_advanced_builder(self, dashboard_id: Optional[int] = None):
        ...
        if dashboard_id:
            ...
            st.info("👉 **Sélectionnez un dashboard existant ci-dessous** ou créez-en un nouveau via l'onglet ⚙️ Gestion")
    ...
    def _show_widget_card(self, widget: Dict):
        ...
        with container:
            ...
            with st.container():
                ...
                # Bouton d'ajout
                if st.button(f"➕ Ajouter", key=f"add_{widget['name']}", 
                           help=f"Ajouter {widget['display_name']} au canvas"):
                ...
                for col_idx in range(self.grid_width):
                    with cols[col_idx]:
                        ...
                        if cell_content:
                            ...
                            # Zone de drop vide
                            if st.button("➕", key=f"empty_{row_idx}_{col_idx}", 
                                       help="Ajouter un widget ici"):
    ...
    def _render_canvas_cell(self, widget: Dict, row: int, col: int):
        ...
        """
        widget_info = next(
            (w for w in DashboardService.get_available_widgets() 
             if w['name'] == widget['widget_type']), 
    ...
    def _show_widget_properties(self):
        ...
        widget_info = next(
            (w for w in DashboardService.get_available_widgets() 
             if w['name'] == selected_widget['widget_type']), 
        ...
        with st.form(f"widget_props_{selected_widget['id']}"):
            ...
            col1, col2 = st.columns(2)
            with col1:
                new_x = st.number_input("Position X", value=selected_widget['position_x'], 
                                      min_value=0, max_value=self.grid_width-1, key="builder_widget_pos_x")
                new_width = st.number_input("Largeur", value=selected_widget.get('width', 1), 
                                          min_value=1, max_value=6, key="builder_widget_width")
            
            with col2:
                new_y = st.number_input("Position Y", value=selected_widget['position_y'], 
                                      min_value=0, max_value=self.grid_height-1, key="builder_widget_pos_y")
                new_height = st.number_input("Hauteur", value=selected_widget.get('height', 1), 
                                           min_value=1, max_value=4, key="builder_widget_height")
            ...
            if st.form_submit_button("💾 Appliquer les changements"):
                self._update_widget_properties(
                    selected_widget['id'],
    ...
    def _render_widget_config_form(self, widget_type: str, current_config: Dict) -> Dict:
        ...
        if widget_type in ['intercontrat_rate', 'intercontrat_trend']:
            ...
            config['show_target'] = st.checkbox(
                ...
                value=config.get('show_target', True),
                key="builder_widget_show_target"
            )
        ...
        # Période par défaut
        if st.checkbox("Configuration de période", value='period_months' in config, key="builder_widget_period_checkbox"):
    ...
    def _remove_widget_from_canvas(self, widget_id: str):
        ...
        """
        st.session_state.canvas_widgets = [
            w for w in st.session_state.canvas_widgets 
            if w['id'] != widget_id
        ]
    ...
    def _update_widget_properties(self, widget_id: str, x: int, y: int, 
    ...
    def _show_builder_actions(self, mode: str, dashboard_id: Optional[int], 
                ...
                if st.form_submit_button("💾 Sauvegarder", type="primary"):
                    if dashboard_name.strip():
                        self._execute_dashboard_save(
                            dashboard_name.strip(), description.strip(),
                            role_access, is_template
                        )
                    else:
    ...
    def _execute_dashboard_save(self, name: str, description: str, 
                               role_access: str, is_template: bool):
        """
