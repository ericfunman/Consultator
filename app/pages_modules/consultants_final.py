...
def show_consultant_skills(consultant):
    ...
    # Onglets pour organiser les fonctionnalités
    tab1, tab2 = st.tabs(["📋 Compétences actuelles", "➕ Ajouter des compétences"])

    with tab1:
        if all_technologies:
            # Récupérer le référentiel de technologies pour vérifier si elles sont
            # connues
            try:
            ...
            with col3:
                st.metric("✋ Ajoutées manuellement", len(competences_manuelles))
        ...
        if submitted:
            if not prenom or not nom or not email:
                ...
                try:
                    ...
                    nouveau_consultant = (
                        ConsultantService.create_consultant(  # noqa: F841
                            consultant_data
                        )
                    )
