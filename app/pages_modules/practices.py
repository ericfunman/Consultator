                                ...
                                consultant_data.append(
                                    {
                                        ...
                                        "Salaire": (
                                            f"{getattr(consultant, 'salaire_actuel', 0):,.0f}€"
                                            if getattr(
                            ...
                            if consultant_data:
                                if pd is not None:
                                    ...
                                    st.dataframe(
                                        ...
                                        column_config={
                                            ...
                                            "Disponible": st.column_config.TextColumn(
                                                "Disponible", width="small"
                                            ),
                                            "Salaire": st.column_config.TextColumn(
                                                "Salaire", width="medium"
                                            ),
                                        },
                                    )
                                else:
                                    ...
                                    for data in consultant_data:
                                        st.write(
                                            f"**{data['Prénom']} {data['Nom']}** - {data['Grade']} - {data['Email']} - {data['Salaire']}"
                                        )
                                ...
                                if st.button(
                                    "🗑️ Retirer de la practice", key="remove_consultant"
                                ):
                                    try:
                                        ...
                                        if success:
                                            ...
                                            st.error(
                                                "❌ Erreur lors du retrait du consultant"
                                            )
                                    except Exception as e:
                                        st.error(f"❌ Erreur: {e}")
                        else:
                    ...
                    if available_consultants:
                        ...
                        if consultant_to_add:
                            ...
                            with col_info1:
                                ...
                                st.write(
                                    f"**Grade:** {getattr(consultant_to_add, 'grade', 'Non défini')}"
                                )
                            with col_info2:
                                current_practice = "Aucune"
                                if consultant_to_add.practice_id:
                                    current_practice_obj = (
                                        PracticeService.get_practice_by_id(
                                            consultant_to_add.practice_id
                                        )
                                    )
                                ...
                                st.write(
                                    f"**Salaire:** {getattr(consultant_to_add, 'salaire_actuel', 0):,.0f}€"
                                )
                        ...
                        if st.button("➕ Affecter à la practice", key="add_consultant"):
                            try:
                                ...
                                if success:
                                    ...
                                    st.error(
                                        "❌ Erreur lors de l'affectation du consultant"
                                    )
                            except Exception as e:
                                st.error(f"❌ Erreur: {e}")
                    else:
                ...
                with tab3:
                    ...
                    with col_action1:
                        ...
                        try:
                            ...
                            if practice_stats:
                                st.metric(
                                    "Total consultants",
                                    practice_stats["total_consultants"],
                                )
                                st.metric(
                                    "Consultants actifs",
                                    practice_stats["consultants_actifs"],
                                )
                                st.write(
                                    f"**Responsable:** {practice_stats['responsable']}"
                                )
                        except Exception as e:
                    ...
                    with col_action2:
                        ...
                        if st.button("📊 Exporter la liste (CSV)", key="export_csv"):
                            try:
                                consultants_dict = (
                                    PracticeService.get_consultants_by_practice(
                                        selected_practice.id
                                    )
                                )
