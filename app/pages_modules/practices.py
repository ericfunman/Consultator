...
def show_practice_overview_optimized():
    ...
    with col3:
        ...
        taux_activite = (consultants_actifs / stats["total_consultants"] * 100) if stats["total_consultants"] > 0 else 0
        st.metric(
    ...
    if stats["practices_detail"]:
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "Total Consultants": st.column_config.NumberColumn("Total", width="small"),
                "Consultants Actifs": st.column_config.NumberColumn("Actifs", width="small"),
...
def show_consultants_by_practice_optimized():
    ...
    if consultants:
        ...
        st.info(f"📊 {total} consultant(s) trouvé(s) - Page {st.session_state.practice_page}/{total_pages}")
        ...
        if total_pages > 1:
            ...
            with col1:
                if st.button("⏮️ Première", disabled=(st.session_state.practice_page == 1)):
                    st.session_state.practice_page = 1
                    st.rerun()
            
            with col2:
                if st.button("⬅️ Précédente", disabled=(st.session_state.practice_page == 1)):
                    st.session_state.practice_page -= 1
                    st.rerun()
            ...
            with col4:
                if st.button("➡️ Suivante", disabled=(st.session_state.practice_page == total_pages)):
                    st.session_state.practice_page += 1
                    st.rerun()
            
            with col5:
                if st.button("⏭️ Dernière", disabled=(st.session_state.practice_page == total_pages)):
        ...
        with st.expander(f"⚡ Actions rapides - {practice_name}"):
            ...
            with col2:
                if st.button(f"📧 Exporter emails", key=f"export_{practice_name}"):
                    emails = [c["email"] for c in consultants if c["email"] and c["email"] != "Non renseigné"]
                    if emails:
                        st.code("; ".join(emails))
...
def show_practice_detailed_stats_cached(practice_name: str):
    ...
    with st.spinner("Calcul des statistiques..."):
        stats = PracticeServiceOptimized.get_practice_detailed_stats_cached(practice_name)
