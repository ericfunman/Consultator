...
class Consultant(Base):
    ...
    # Date de sortie (peut être null si encore en poste)
    date_sortie_societe = Column(Date)
    ...
    type_contrat = Column(String(20), default='CDI')
    ...
    salaires = relationship(
        "ConsultantSalaire",
        back_populates="consultant",
        cascade="all, delete-orphan")
    langues = relationship(
        "ConsultantLangue",
        back_populates="consultant",
        cascade="all, delete-orphan")
    business_manager_gestions = relationship(
...
class Competence(Base):
    ...
    type_competence = Column(String(20), default='technique')
    description = Column(Text)
...
class ConsultantCompetence(Base):
    ...
    niveau_maitrise = Column(String(20), default='debutant')
    ...
    def __repr__(self):
        return f"<ConsultantCompetence(consultant_id={
            self.consultant_id}, competence_id={
            self.competence_id}, experience={
...
class ConsultantSalaire(Base):
    ...
    def __repr__(self):
        return f"<ConsultantSalaire(id={
            self.id}, consultant_id={
            self.consultant_id}, salaire={
            self.salaire}, date_debut={
                self.date_debut})>"
...
class ConsultantLangue(Base):
    ...
    def __repr__(self):
        return f"<ConsultantLangue(consultant_id={
            self.consultant_id}, langue_id={
            self.langue_id}, niveau={
            self.niveau})>"
...
class BusinessManager(Base):
    ...
    # Relations
    consultant_gestions = relationship(
        "ConsultantBusinessManager",
        back_populates="business_manager",
        cascade="all, delete-orphan")

    def __repr__(self):
        return f"<BusinessManager(id={
            self.id}, nom='{
            self.nom}', prenom='{
            self.prenom}')>"
...
class ConsultantBusinessManager(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    consultant_id = Column(Integer, ForeignKey('consultants.id'), nullable=False)
    business_manager_id = Column(
        Integer,
        ForeignKey('business_managers.id'),
        nullable=False)
    date_debut = Column(Date, nullable=False, default=datetime.now)
    ...
    business_manager = relationship(
        "BusinessManager",
        back_populates="consultant_gestions")

    def __repr__(self):
        return f"<ConsultantBusinessManager(consultant_id={
            self.consultant_id}, bm_id={
            self.business_manager_id}, debut={
            self.date_debut})>"
