...
class DashboardService:
    ...
    def get_all_dashboards() -> List[Dict]:
        ...
        try:
            with get_database_session() as session:
                dashboards = session.query(DashboardConfiguration).order_by(
                    DashboardConfiguration.is_template.desc(),
                    DashboardConfiguration.nom
                ).all()
    ...
    def get_dashboard_by_id(dashboard_id: int) -> Optional[Dict]:
        ...
        try:
            with get_database_session() as session:
                dashboard = session.query(DashboardConfiguration).filter(
    ...
    @staticmethod
    def create_dashboard(nom: str, description: str = "", role_access: str = "all", 
                        created_by: str = "system", is_template: bool = False) -> Optional[int]:
        """
    ...
    @staticmethod
    def add_widget_to_dashboard(dashboard_id: int, widget_type: str, 
    ...
    def remove_widget_from_dashboard(widget_instance_id: int) -> bool:
        ...
        try:
            with get_database_session() as session:
                ...
                widget_instance = session.query(DashboardWidgetInstance).filter(
                    DashboardWidgetInstance.id == widget_instance_id
                ).first()
    ...
    def get_available_widgets() -> List[Dict]:
        ...
        try:
            with get_database_session() as session:
                widgets = session.query(WidgetCatalog).filter(
                    WidgetCatalog.is_active == True
                ).order_by(WidgetCatalog.category, WidgetCatalog.display_name).all()
...
class DashboardDataService:
    ...
    def get_intercontrat_data(bm_filter: Optional[int] = None) -> Dict:
        ...
        try:
            with get_database_session() as session:
                ...
                if bm_filter:
                    consultant_ids_bm = session.query(ConsultantBusinessManager.consultant_id).filter(
                        and_(
                ...
                consultants_avec_mission = query_consultants.filter(
                    Consultant.id.in_(
                        session.query(Mission.consultant_id).filter(Mission.statut == 'en_cours')
                    )
                ).count()
    ...
    def get_revenue_by_bm_data(period_months: int = 3) -> Dict:
        ...
        try:
            with get_database_session() as session:
                ...
                ).filter(BusinessManager.actif == True).group_by(
    ...
    def _get_last_mission_date(consultant_id: int, session: Session) -> Optional[str]:
        ...
        """
        try:
            last_mission = session.query(Mission).filter(
                Mission.consultant_id == consultant_id,
                Mission.statut == 'termine'
            ).order_by(desc(Mission.date_fin)).first()
