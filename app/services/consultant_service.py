...
class ConsultantService:
    ...
    def get_consultant_by_id(consultant_id: int) -> Optional[Consultant]:
        ...
        try:
            with get_database_session() as session:
                ...
                # Détacher l'objet de la session pour éviter les erreurs de session fermée
                if consultant:
                    session.expunge(consultant)
    ...
    def get_consultant_with_stats(consultant_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"Erreur lors de la récupération du consultant avec stats {consultant_id}: {e}")
            return None
    ...
    def get_consultant_by_email(email: str) -> Optional[Consultant]:
        ...
        try:
            ...
            print(f"Erreur lors de la récupération du consultant par email {email}: {e}")
            return None
        ...
        try:
            with get_database_session() as session:
                ...
                for mission_data in analysis_data.get('missions', []):
                    if ConsultantService._save_mission_from_analysis(session, consultant_id, mission_data):
                        missions_saved += 1
                ...
                for tech in analysis_data.get('langages_techniques', []):
                    if ConsultantService._save_competence_from_analysis(session, consultant_id, tech, 'technique'):
                        skills_saved += 1
                
                # Compétences fonctionnelles
                for func_skill in analysis_data.get('competences_fonctionnelles', []):
                    if ConsultantService._save_competence_from_analysis(session, consultant_id, func_skill, 'fonctionnelle'):
                        skills_saved += 1
                ...
                session.commit()
                print(f"✅ Analyse sauvegardée: {missions_saved} missions, {skills_saved} compétences")
                return True
    ...
    @staticmethod
    def _save_competence_from_analysis(session: Session, consultant_id: int, competence_name: str, type_competence: str) -> bool:
    ...
    def _determine_skill_category(skill_name: str, type_competence: str) -> str:
        ...
        if type_competence == 'fonctionnelle':
            if any(word in skill_lower for word in ['management', 'leadership', 'gestion', 'direction']):
                return 'Management'
        ...
        # Compétences techniques
        frontend_keywords = ['react', 'angular', 'vue', 'javascript', 'typescript', 'html', 'css']
        backend_keywords = ['java', 'python', 'spring', 'django', 'flask', 'node.js', 'express']
        database_keywords = ['sql', 'mysql', 'postgresql', 'mongodb', 'oracle', 'redis']
        cloud_keywords = ['aws', 'azure', 'gcp', 'docker', 'kubernetes', 'cloud']
        devops_keywords = ['jenkins', 'gitlab', 'github', 'ci/cd', 'terraform', 'ansible']
                ...
                for skill_name in technical_skills:
                    ...
                    if not competence:
                        ...
                        competence = Competence(
                            ...
                            categorie=ConsultantService._determine_skill_category(skill_name, 'technique'),
                ...
                for skill_name in functional_skills:
                    ...
                    if not competence:
                        ...
                        competence = Competence(
                            ...
                            categorie=ConsultantService._determine_skill_category(skill_name, 'fonctionnelle'),
