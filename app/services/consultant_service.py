...
class ConsultantService:
    ...
    def get_consultant_by_id(consultant_id: int) -> Optional[Consultant]:
        ...
        try:
            ...
            print(
                f"Erreur lors de la récupération du consultant {consultant_id}: {e}"
            )
            return None

    @staticmethod
    def get_consultant_with_stats(
        consultant_id: int,
        ...
        try:
            with get_database_session() as session:
                ...
                competences_count = (
                    len(consultant.competences)
                    if consultant.competences
                    else 0
                )
                missions_count = (
                    len(consultant.missions) if consultant.missions else 0
                )
        ...
        try:
            with get_database_session() as session:
                consultant = Consultant(
                    ...
                    salaire_actuel=data.get(
                        "salaire"
                    ),  # Corrigé: salaire_actuel au lieu de salaire
                    disponibilite=data.get(
                        "disponible", True
                    ),  # Corrigé: disponibilite au lieu de disponible
    ...
    def update_consultant(consultant_id: int, data: dict) -> bool:
        ...
        try:
            with get_database_session() as session:
                ...
                if not consultant:
                    print(
                        f"❌ Consultant avec l'ID {consultant_id} non trouvé"
                    )
                    return False
    ...
    def get_available_consultants() -> List[Consultant]:
        ...
        try:
            ...
            print(
                f"Erreur lors de la récupération des consultants disponibles: {e}"
            )
            return []

    @staticmethod
    def save_cv_analysis(
        consultant_id: int, analysis_data: Dict[str, Any]
    ) -> bool:
        ...
        try:
            with get_database_session() as session:
                ...
                for func_skill in analysis_data.get(
    ...
    def _save_mission_from_analysis(
        ...
        try:
            ...
            if date_debut:
                ...
                if existing:
                    print(
                        f"Mission {client} {date_debut} déjà existante, ignorée"
                    )
                    return False
            ...
            print(
                f"Erreur sauvegarde mission {mission_data.get('client', 'N/A')}: {e}"
            )
            return False
    ...
    def _determine_skill_category(
        skill_name: str, type_competence: str
    ) -> str:
        ...
        if type_competence == "fonctionnelle":
            if any(
                ...
                word in skill_lower
                for word in ["scrum", "agile", "kanban", "projet"]
            ):
                return "Methodologie"
            elif any(
                word in skill_lower
    ...
    def save_cv_analysis(
        ...
        try:
            with get_database_session() as session:
                ...
                if not consultant:
                    st.error(
                        f"❌ Consultant avec ID {consultant_id} introuvable"
                    )
                    return False
                ...
                for mission_data in missions:
                    ...
                    if not existing_mission:
                        ...
                        new_mission = Mission(
                            ...
                            date_fin=date_fin,
                            statut=(
                                "en_cours" if date_fin is None else "terminee"
                            ),
                            technologies_utilisees=", ".join(
                                mission_data.get("langages_techniques", [])
                            ),
                        ...
                        missions_count += 1
                        st.success(
                            f"✅ Mission ajoutée: {mission_data['client']}"
                        )
                ...
                for skill_name in technical_skills:
                    ...
                    if not existing_skill:
                        ...
                        skills_count += 1
                        st.success(
                            f"✅ Compétence technique ajoutée: {skill_name}"
                        )

                # 3. Sauvegarder les compétences fonctionnelles
                functional_skills = analysis_data.get(
                    "competences_fonctionnelles", []
                )
                for skill_name in functional_skills:
                    ...
                    if not existing_skill:
                        ...
                        skills_count += 1
                        st.success(
                            f"✅ Compétence fonctionnelle ajoutée: {skill_name}"
                        )
