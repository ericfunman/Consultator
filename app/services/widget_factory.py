...
class WidgetFactory:
    ...
    def _render_intercontrat_rate(config: Dict) -> None:
        ...
        with col1:
            st.metric(
                "Taux d'intercontrat",
                f"{taux}%",
                help="Pourcentage de consultants sans mission en cours"
            )
    ...
    def _render_global_kpis(config: Dict) -> None:
        ...
        with col2:
            st.metric(
                ...
                delta=f"-{intercontrat_data['taux_intercontrat'] - 12:.1f}%" if intercontrat_data['taux_intercontrat'] > 12 else None,
                delta_color="inverse",
        ...
        with col3:
            ...
            st.metric(
                "🎯 Missions Actives",
                total_missions,
                help="Nombre de missions en cours ce mois"
            )
        ...
        with col4:
            ...
            st.metric(
                "💰 CA Mensuel",
                f"{ca_mensuel:,.0f} €",
                help="Chiffre d'affaires estimé du mois"
            )
    ...
    def _render_intercontrat_trend(config: Dict) -> None:
        ...
        df_trend = pd.DataFrame({
            'Date': dates[:len(taux_simules)],
            'Taux': taux_simules[:len(dates)]
        })
        ...
        fig = px.line(
            ...
            x='Date',
            y='Taux',
            title="Évolution du Taux d'Intercontrat",
            markers=True
        )
        ...
        fig.update_layout(
            yaxis_title="Taux d'Intercontrat (%)",
            height=300
        )
        ...
        try:
            with get_database_session() as session:
                for widget_data in widgets_to_create:
                    ...
                    existing = session.query(WidgetCatalog).filter(
                        WidgetCatalog.name == widget_data['name']
                    ).first()
