...
class DocumentAnalyzer:
    ...
    CLIENTS_CONNUS = [
        # ESN et Conseil
        'Quanteam', 'Rainbow Partners', 'Capgemini', 'Accenture', 'Sopra Steria', 
        'Atos', 'CGI', 'IBM', 'TCS', 'Deloitte', 'PwC', 'KPMG', 'EY',
        'Wavestone', 'Sia Partners', 'Mc Kinsey', 'Boston Consulting',
        
        # Banques & Finance
        'BNP Paribas', 'BNPP', 'Société Générale', 'SG', 'SGCIB', 'Crédit Agricole', 
        'BPCE', 'Natixis', 'Crédit Mutuel', 'La Banque Postale',
        'Amundi', 'Axa Investment', 'CNP Assurances',
        
        # Assurances
        'AXA', 'CNP Assurances', 'Generali', 'Allianz', 'Swiss Life',
        'Groupama', 'MAAF', 'Matmut', 'MACIF',
        
        # Télécoms
        'Orange', 'SFR', 'Bouygues Telecom', 'Free', 'Altice',
        
        # Automobile
        'Renault', 'PSA', 'Stellantis', 'Peugeot', 'Citroën', 'Michelin', 'Valeo',
        
        # Énergie
        'Total', 'TotalEnergies', 'Engie', 'EDF', 'Veolia', 'Suez',
        
        # Transport & Aéronautique
        'SNCF', 'RATP', 'Airbus', 'Safran', 'Thales', 'Dassault',
        
        # Tech & Digital
        'Amazon', 'Microsoft', 'Google', 'Meta', 'Apple', 'Salesforce',
        'SAP', 'Oracle', 'IBM', 'VMware',
        
        # Services publics
        'La Poste', 'Pôle Emploi', 'CAF', 'CPAM', 'URSSAF',
        
        # Distribution
        'FNAC', 'Carrefour', 'Leclerc', 'Auchan', 'Casino',
        
        # Autres
        'LVMH', 'L\'Oréal', 'Danone', 'Schneider Electric', 'Legrand'
    ]
    
    # Termes indiquant une mission actuelle
    CURRENT_MISSION_INDICATORS = [
        'en cours', 'actuel', 'présent', 'aujourd\'hui', 'maintenant',
        'depuis', 'toujours en poste', 'current', 'ongoing',
    ...
    DATA_TECHNOLOGIES = [
        # Langages Data
        'Python', 'R', 'SQL', 'Scala', 'Java', 'SAS', 'SPSS',
        
        # Big Data & Streaming
        'Hadoop', 'Spark', 'Kafka', 'Flink', 'Storm', 'Kinesis',
        'Databricks', 'Snowflake', 'BigQuery', 'Redshift',
        
        # ETL & Orchestration  
        'Airflow', 'Luigi', 'Talend', 'Informatica', 'SSIS', 'Pentaho',
        'NiFi', 'Glue', 'Data Factory',
        
        # Bases de données
        'PostgreSQL', 'MySQL', 'Oracle', 'SQL Server', 'MongoDB',
        'Cassandra', 'HBase', 'DynamoDB', 'Elasticsearch', 'Redis',
        
        # BI & Visualisation
        'Power BI', 'Tableau', 'QlikView', 'QlikSense', 'Looker', 'Grafana',
        'Superset', 'Metabase', 'SAS VA', 'Cognos',
        ...
        # Finance spécialisé
        'SWIFT', 'FIX Protocol', 'Bloomberg API', 'Reuters', 'Risk Management',
        'Basel III', 'IFRS', 'Solvency II', 'MiFID II',
        'Central Bank Reporting', 'BALE II', 'OST', 'TPS'
    ]
    ...
    def analyze_cv_content(text: str, consultant_name: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            st.success(f"✅ Analyse terminée: {len(missions)} missions, {len(analysis['langages_techniques'])} technologies")
    ...
    def _extract_missions(text: str) -> List[Dict]:
        ...
        experience_keywords = [
            'expérience professionnelle', 'parcours professionnel', 'historique professionnel',
            'expériences', 'missions', 'emplois', 'postes', 'carrière', 'activités professionnelles',
            'réalisations', 'projets', 'interventions', 'mandats', 'consulting', 'conseil'
        ]
        ...
        st.info(f"🚀 {len(missions)} missions brutes trouvées avant nettoyage (dont {len(missions_powerpoint)} PowerPoint optimisées et {len(missions_quanteam)} Quanteam corrigées)")
        ...
        sorted_powerpoint_missions = sorted(powerpoint_missions, key=lambda x: DocumentAnalyzer._date_sort_key(x.get('date_debut', '')), reverse=False)
        
        # Trier les autres missions par date de début (plus récent en premier)
        sorted_other_missions = sorted(other_missions, key=lambda x: DocumentAnalyzer._date_sort_key(x.get('date_debut', '')), reverse=True)
        
        # Combiner : PowerPoint optimisées triées chronologiquement, puis les autres
        final_missions = sorted_powerpoint_missions + sorted_other_missions[:10]  # PowerPoint triées + 10 autres max
        
        st.success(f"✅ {len(final_missions)} missions finales ({len(powerpoint_missions)} PowerPoint optimisées + {len(sorted_other_missions[:10])} autres)")
    ...
    def _find_dates_in_text_improved(text: str) -> List[str]:
        ...
        date_patterns = [
            ...
            (r'\b(\d{4})\s*[-–—]\s*(en\s+cours|actuel|présent|aujourd\'hui)\b', 'ongoing'),  # 2020-en cours
            (r'\bdepuis\s+(\d{4})\b', 'since'),  # depuis 2020
            (r'\ben\s+(\d{4})\b', 'year'),  # en 2020
            
            # Mois en français - AMÉLIORATION CLÉE POUR QUANTEAM
            (r'\b(janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+(\d{4})\s*[-–—]\s*(en\s+cours|actuel|présent|aujourd\'hui)', 'month_current'),
            (r'\b(janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+(\d{4})\b', 'month_fr'),
            (r'\b(jan|fév|mar|avr|mai|jun|jul|aoû|sep|oct|nov|déc)\.?\s+(\d{4})\b', 'month_abbr'),
            
            # Patterns spécifiques pour missions actuelles
            (r'\b(\d{4})\s*[-–—]\s*à\s+ce\s+jour\b', 'to_date'),
            (r'\bdepuis\s+(janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+(\d{4})\b', 'since_month'),
            (r'\b(janvier|février|mars|avril|mai|juin|juillet|août|septembre|octobre|novembre|décembre)\s+(\d{4})\s*[-–—]\s*$', 'month_ongoing'),
        ]
    ...
    def _extract_long_mission_summary(text: str) -> str:
        ...
        mission_keywords = [
            'mission', 'projet', 'développement', 'conception', 'réalisation', 'mise en place',
            'création', 'analyse', 'étude', 'design', 'architecture', 'implémentation',
            'déploiement', 'maintenance', 'support', 'optimisation', 'migration',
            'formation', 'conseil', 'audit', 'expertise', 'accompagnement', 'pilotage'
        ]
        ...
        for sentence in sentences:
            ...
            if 20 < len(sentence) < 500:
                ...
                action_verbs = ['développé', 'créé', 'conçu', 'réalisé', 'mis en place', 'déployé']
    ...
    def get_analysis_preview(analysis_data: Dict) -> str:
        ...
        if missions:
            ...
            for i, mission in enumerate(missions, 1):
                ...
                preview.append(f"  {i}. **{mission['client']}**{role_info} ({mission['date_debut']} → {mission['date_fin']})")
    ...
    def _extract_missions_company_date_role_format(text: str) -> List[Dict]:
        ...
        for i in range(len(lines) - 2):  # -2 pour avoir au moins 3 lignes
            ...
            if (line1 and  # Ligne entreprise non vide
                re.match(r'\w+\s+\d{4}\s*[–-]', line2) and  # Ligne date avec année
                ('directeur' in line3.lower() or 'chef' in line3.lower() or 'manager' in line3.lower() or 
                 'lead' in line3.lower() or 'responsable' in line3.lower())):  # Ligne poste
                
                # Cas spécial pour Quanteam
                if line1.lower() == 'quanteam':
                    st.success(f"🎯 Quanteam détecté! Date: '{line2}', Poste: '{line3}'")
                    ...
                    if date_match:
                        ...
                        for j in range(i + 3, min(i + 10, len(lines))):
                            if lines[j].strip() and not re.match(r'\w+\s+\d{4}', lines[j]):
                                description_lines.append(lines[j].strip())
                            else:
                                break
                        
                        description = '\n'.join(description_lines[:7])  # Limiter à 7 lignes
                        ...
                        missions.append(mission)
                        st.success(f"✅ Mission Quanteam ajoutée: {date_debut} - {line3}")
                
                # Autres entreprises connues
                elif (any(known_client.lower() in line1.lower() for known_client in DocumentAnalyzer.CLIENTS_CONNUS) or 
                      len(line1.split()) <= 3):  # Nom court probable
                    
                    st.info(f"🔍 Entreprise détectée: '{line1}' | '{line2}' | '{line3}'")
                    ...
                    tech_in_desc = DocumentAnalyzer._extract_technical_skills(description)
        ...
        for pattern in quanteam_patterns:
            ...
            for match in matches:
                ...
                mission = {
                    ...
                    'langages_techniques': ['Data Management', 'Practice Management'],
                    'competences_fonctionnelles': ['Management', 'Commercial', 'Recrutement'],
