...
def add_table_of_contents(doc, toc_sections):
    ...
    doc.add_paragraph("")
    update_note = doc.add_paragraph("💡 Pour mettre à jour automatiquement : Clic droit sur la table des matières > Mettre à jour les champs", style='NoteStyle')
...
def should_skip_line(line):
    ...
    if line in ['|', '.. grid::', '   :gutter:', '   .. grid-item-card::', '      :link:', '      :link-type:']:
        return True
...
def convert_rst_to_docx_improved(rst_content, doc, section_title="", toc_sections=None, is_index=False):
    ...
    while i < len(lines):
        ...
        if i + 1 < len(lines) and lines[i + 1].startswith(('=', '-', '~', '^', '"')):
            ...
            if underline == '=' and section_title and title_text.lower() == section_title.lower():
                i += 2
                continue
...
def create_professional_word_documentation():
    ...
    if index_path.exists():
        ...
        convert_rst_to_docx_improved(content, doc, "Page d'accueil", toc_sections, is_index=True)
