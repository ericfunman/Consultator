...
def add_clickable_table_of_contents(doc, toc_sections):
    ...
    toc_instructions = doc.add_paragraph(
        "📋 Table des matières automatique :", style="Heading2Custom"
    )
...
def should_skip_line(line, in_grid_section=False):
    ...
    if in_grid_section and (
        line.startswith("      ") and not line.startswith("      :")
    ):
        return True

    # Tableaux de métriques (éviter qu'ils aillent dans TOC)
    if line.startswith(".. list-table::") and (
        "Métriques" in line or "Ressources" in line
    ):
        return True
...
def convert_rst_to_docx_perfect(rst_content, doc, section_title="", is_index=False):
    ...
    while i < len(lines):
        ...
        if (
            line.startswith(".. list-table::")
            and "Métriques" not in line
            and "Ressources" not in line
        ):
