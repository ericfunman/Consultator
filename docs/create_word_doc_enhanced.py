...
def setup_document_styles(doc):
    ...
    code_style.paragraph_format.shading = parse_xml(r'<w:shd {} w:fill="F8F8F8"/>'.format(nsdecls('w')))
    ...
    note_style.paragraph_format.shading = parse_xml(r'<w:shd {} w:fill="FFFFE0"/>'.format(nsdecls('w')))
...
def convert_rst_to_docx(rst_content, doc):
    ...
    while i < len(lines):
        ...
        if line.startswith('.. list-table::') or (line.strip() == '' and i > 0 and lines[i-1].startswith('.. list-table::')):
        ...
        if line.startswith('.. code-block::') or (i > 0 and lines[i-1].startswith('.. code-block::')):
            code_lines = []
            j = i
        ...
        if line.startswith('.. ') and not any(line.startswith(f'.. {directive}::')
...
def create_enhanced_word_documentation():
    ...
    for filename, section_title in files_order:
        ...
        if filepath.exists():
            ...
            if lines and lines[0].strip() and len(lines) > 1 and lines[1].startswith('='):
