...
class SonarIntegration:
    ...
    def __init__(self, sonar_url="http://localhost:9000", project_key="consultator-app"):
    ...
    def generate_quality_report(self):
                ...
                if "Your code has been rated at" in content:
                    ...
                    if score_line:
                        ...
                        report["reports"].append({
                            "tool": "Pylint",
                            "score": float(score),
                            "status": "PASSED" if float(score) >= 8.0 else "WARNING"
                        })
        ...
        report_file = self.reports_dir / f"quality-report-{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
