...
def show_final_statistics():
    ...
    with get_database_session() as session:
        ...
        print("\n🏢 RÉPARTITION PAR PRATIQUE:")
        practices_stats = (
            session.query(Practice.nom, func.count(Consultant.id))
            .join(Consultant)
            .group_by(Practice.nom)
            .order_by(func.count(Consultant.id).desc())
            .all()
        )
        ...
        for c in consultants:
            ...
            bm_name = (
                c.business_manager_actuel.nom_complet
                if c.business_manager_actuel
                else "Aucun"
            )
            print(f"  - {c.prenom} {c.nom} ({c.email})")
            print(
                f"    Société: {c.societe} | Pratique: {practice_name} | BM: {bm_name}"
            )
