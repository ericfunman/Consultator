...
MSG_CONSULTANT_NON_TROUVE = (
    "⚠️ Consultant avec user_id {} non trouvé, mission ignorée: {}"
)
...
def validate_mission_data(row: pd.Series) -> Dict:
    ...
    try:
        ...
        try:
            mission_data["cjm"] = (
                float(row.get("CJM", 0)) if pd.notna(row.get("CJM")) else None
            )
        except (ValueError, TypeError):
...
def import_vsa_missions(
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    consultant = (
                        ...
                        .filter(Consultant.id == mission_data["user_id"])
                        .first()
                    )

                    if not consultant:
                        print(
                            MSG_CONSULTANT_NON_TROUVE.format(
                    ...
                    existing_mission = (
                        ...
                        .filter(VSA_Mission.code == mission_data["code"])
                        .first()
                    )
    ...
    print(
        MSG_IMPORT_TERMINE.format(stats["imported"], stats["errors"], stats["skipped"])
    )
    return stats
...
def show_import_summary(stats: Dict[str, int]) -> None:
    ...
    print(
        f"📈 Taux de succès: {(stats['imported'] / max(stats['imported'] + stats['errors'] + stats['skipped'], 1) * 100):.1f}%"
    )
