...
def create_quality_charts():
    ...
    bars2 = ax1.bar(
        x + width / 2, after, width, label="Après nettoyage", color="#90ee90", alpha=0.8
    )
    ...
    bars1 = ax.bar(
        x - width, security_scores, width, label="Sécurité", color="#ff6b6b", alpha=0.8
    )
    ...
    ax1.text(
        0, 0, f"{score}/100", ha="center", va="center", fontsize=20, fontweight="bold"
    )
    ...
    bars = ax4.bar(
        performance_data, performance_values, color=performance_colors, alpha=0.8
    )
    ...
    for bar in bars:
        height = bar.get_height()
        ax4.annotate(
            f"{height}%" if height != 48 else "48s",
            xy=(bar.get_x() + bar.get_width() / 2, height),
            xytext=(0, 3),
            textcoords="offset points",
            ha="center",
...
def create_enhanced_quality_report():
    ...
    executive_summary.add_run(
        "🟢 ULTRA-EXCELLENT - Application certifiée qualité professionnelle\n\n"
    )
    ...
    doc.add_paragraph(
        "Amélioration spectaculaire après suppression des fichiers de backup :"
    )
    ...
    footer_para.add_run(
        "🔧 Outils : Bandit, Flake8, PyLint, Pytest, Matplotlib\n"
    ).italic = True
    footer_para.add_run(
        f'📊 Graphiques : Générés automatiquement le {datetime.now().strftime("%d/%m/%Y")}\n'
    ).italic = True
