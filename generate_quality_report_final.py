...
class CodeQualityAnalyzer:
    ...
    def analyze_code_complexity(self) -> Dict[str, int]:
        ...
        for file_path in self.python_files:
            try:
                ...
                if complexity > 0:
                    complexity_results[str(file_path.relative_to(self.project_root))] = complexity

            except Exception as e:
                print(f"Erreur lors de l'analyse de {file_path}: {e}")

        return dict(sorted(complexity_results.items(), key=lambda x: x[1], reverse=True))
            ...
            try:
                ...
                for i, line in enumerate(lines, 1):
                    if 'f"' in line and '{' not in line:
                        file_issues.append(f"Ligne {i}: f-string inutile sans variables")

                # Vérifier les constantes dupliquées
                duplicate_literals = ['Non affecté', 'En cours', 'N/A', '✅ Disponible', '🔴 Occupé']
                for literal in duplicate_literals:
                    if content.count(f"'{literal}'") > 2:
                        file_issues.append(f"Littéral '{literal}' dupliqué plusieurs fois")

                # Vérifier la longueur des lignes
                long_lines = [i for i, line in enumerate(lines, 1) if len(line) > 100]
                if long_lines:
                    file_issues.append(f"{len(long_lines)} lignes trop longues (>100 caractères)")
                ...
                for i, line in enumerate(lines, 1):
                    if line.strip().startswith('def '):
                        if len(current_function) > 50:
                            file_issues.append(f"Fonction trop longue détectée autour de la ligne {i}")
                        current_function = [i]
                    elif line.strip() and not line.startswith(' ') and not line.startswith('\t'):
                        if current_function and len(current_function) > 50:
                            file_issues.append(f"Fonction trop longue détectée autour de la ligne {current_function[0]}")
                        current_function = []
                ...
                print(f"Erreur lors de l'analyse des bonnes pratiques de {file_path}: {e}")
    ...
    def generate_report(self) -> str:
        ...
        for file, metric in sorted(metrics.items())[:10]:  # Top 10 fichiers
            ...
            report.append(f"   • Lignes: {metric['total_lines']} ({metric['code_lines']} code)")
            report.append(f"   • Fonctions: {metric['functions']}, Classes: {metric['classes']}")
        ...
        if any(m['comment_ratio'] < 10 for m in metrics.values()):
            recommendations.append("• Augmenter le ratio de commentaires (< 10% dans certains fichiers)")
        if any(m['functions'] > 20 for m in metrics.values()):
            recommendations.append("• Découper les fichiers avec trop de fonctions (> 20)")
        if issues:
            recommendations.append("• Corriger les problèmes de bonnes pratiques détectés")
