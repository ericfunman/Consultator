...
def search_eric_lapina():
    ...
    if not eric_lapina.empty:
        ...
        if classification == 'bm':
            print('⚠️ Ce consultant a été classé comme Business Manager et donc exclu de l\'import des consultants')
            ...
            with get_database_session() as session:
                ...
                existing = session.query(Consultant).filter(Consultant.email == email).first()
                if existing:
                    print(f'✅ Trouvé dans la base: {existing.prenom} {existing.nom} ({existing.email})')
                else:
        ...
        if not eric_rows.empty:
            ...
            for _, row in eric_rows.iterrows():
                print(f'  {row["firstname"]} {row["lastname"]} - {row["email"]} - {row["job_title"]}')
